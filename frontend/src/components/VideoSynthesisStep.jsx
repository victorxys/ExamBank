...
const VideoSynthesisStep = ({ contentId, setSynthesisTask, synthesisTask, allSentences, progressData, isSubmitting, onStartTask, onResetTask, onAlert, onSkipStep
    ...
    const handleExportPdf = async () => {
        ...
        setIsExporting(true);
        try {
            const response = await ttsApi.exportVideoHandoutPdf(synthesisTask.id, exportOrientation);
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            // 找到 content 标题用于文件名
            const safeTitle = "alignment_handout";
            link.setAttribute('download', `${safeTitle}.pdf`);
            document.body.appendChild(link);
            link.click();
            link.remove();
            window.URL.revokeObjectURL(url);
            onAlert({ open: true, message: 'PDF 讲义已开始导出并下载。', severity: 'success' });
        } catch (error) {
            console.error("导出 PDF 失败:", error);
            onAlert({ open: true, message: '导出 PDF 失败，请稍后重试。', severity: 'error' });
        } finally {
