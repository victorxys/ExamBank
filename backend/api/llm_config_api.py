...
def create_llm_model():
    ...
    if (
        ...
        or not data.get("model_identifier")
        or not data.get("provider")
    ):
        return jsonify(
            {"error": "缺少必要字段: model_name, model_identifier, provider"}
        ), 400
    ...
    try:
        ...
        if "uq_llm_model_name" in str(e.orig):
            error_msg = "模型名称已存在"
        elif "uq_llm_model_identifier" in str(e.orig):
            error_msg = "模型标识符已存在"
...
def update_llm_model(model_id):
    ...
    if (
        "model_identifier" in data
        and data["model_identifier"] != model.model_identifier
    ):
        ...
        return jsonify(
            {
                ...
                "updated_at": api_key_record.updated_at.isoformat()
                if api_key_record.updated_at
                else None,
            }
        )
    except Exception as e:  # 其他可能的数据库错误
...
def create_llm_prompt():
    ...
    new_prompt = LlmPrompt(
        ...
        if data.get("model_identifier")
        else None,  # 确保空字符串转为 None
    ...
    try:
        ...
        current_app.logger.error(
            "创建提示词失败: 具有相同标识符和版本的提示词已存在", exc_info=True
        )
...
def get_llm_prompts():
    try:
        prompts = LlmPrompt.query.order_by(
            LlmPrompt.prompt_identifier,
            LlmPrompt.version.desc(),
            LlmPrompt.created_at.desc(),
        ).all()
        return jsonify(
            [
                {
                    ...
                    "model_name": prompt.llm_model_ref.model_name
                    if prompt.llm_model_ref
                    else None,  # 通过关系获取模型名称
...
def update_llm_prompt(prompt_id):
    ...
    if new_identifier != prompt.prompt_identifier or new_version != prompt.version:
        existing_prompt = LlmPrompt.query.filter_by(
            prompt_identifier=new_identifier, version=new_version
        ).first()
    ...
    prompt.model_identifier = (
        data.get("model_identifier") if data.get("model_identifier") else None
    )  # 确保空字符串转为 None
