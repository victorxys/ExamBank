...
def create_substitute_record(contract_id):
    ...
    try:
        ...
        return jsonify({'message': '替班记录已创建，相关账单已更新。', 'record_id': str(new_record.id)}), 201

    except ValueError:
        return jsonify({'error': 'Invalid date or amount format'}), 400
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Failed to create substitute record: {e}", exc_info=True)
...
def get_substitute_records(contract_id):
    try:
        records = SubstituteRecord.query.filter_by(main_contract_id=contract_id).order_by(SubstituteRecord.start_date.desc()).all()
        result = [{
            'id': str(record.id),
            'substitute_user_id': str(record.substitute_user_id),
            'substitute_user_name': record.substitute_user.username if record.substitute_user else 'N/A',
        ...
        current_app.logger.error(f"Failed to get substitute records for contract {contract_id}: {e}", exc_info=True)
        return jsonify({'error': 'Internal server error'}), 500
...
def delete_substitute_record(record_id):
    ...
    try:
        ...
        current_app.logger.info(f"Deleting substitute record {record_id} for contract {sub_record.main_contract_id}, force delete: {force_delete}")
        if force_delete:
            if sub_record.generated_bill_id:
                FinancialActivityLog.query.filter_by(customer_bill_id=sub_record.generated_bill_id).delete(synchronize_session=False)
            if sub_record.generated_payroll_id:
                FinancialActivityLog.query.filter_by(employee_payroll_id=sub_record.generated_payroll_id).delete(synchronize_session=False)

        if sub_record.generated_bill_id:
            CustomerBill.query.filter_by(id=sub_record.generated_bill_id).delete(synchronize_session=False)
        if sub_record.generated_payroll_id:
            EmployeePayroll.query.filter_by(id=sub_record.generated_payroll_id).delete(synchronize_session=False)
        ...
        if 'financial_activity_logs' in str(e.orig):
            ...
            current_app.logger.error(f"Integrity error on delete substitute record {record_id}: {e}", exc_info=True)
            return jsonify({'error': 'Database integrity error'}), 500
            
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Failed to delete substitute record {record_id}: {e}", exc_info=True)
        return jsonify({'error': 'Internal server error'}), 500
