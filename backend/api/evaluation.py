    ...
    try:
        ...
        if evaluation_ids:
            ...
            for row in manual_inputs_raw:
                ...
                manual_inputs_map[eval_id_str][category_id_str].append(
                    row["manual_input"]
                )
            ...
            for category_id, inputs in categories.items():
                ...
                for aspect in aspects_dict.values():
                    if category_id in aspect["_category_dict"]:
                        aspect["_category_dict"][category_id]["manual_inputs"].extend(
                            inputs
                        )
                        category_found = True
                        break
                ...
                if not category_found:
                    ...
                    if cat_info:
                        ...
                        aspects_dict[aspect_name]["_category_dict"][category_id] = (
                            new_category
                        )
            ...
            for category in aspect["categories"]:
                ...
                if category["items"] or category["manual_inputs"]:
                    ...
                    if category["items"]:  # 如果有评分items，计算平均分
                        category["average_score"] = sum(
                            item["average_score"] for item in category["items"]
                        ) / len(category["items"])
...
def update_evaluation(evaluation_id, data):
    ...
    try:
        ...
        # 验证评价记录是否存在
        cur.execute(
            "SELECT id, evaluator_customer_id FROM evaluation WHERE id = %s",
            (evaluation_id,),
        )
        ...
        if item_scores is not None:  # 仅当提供了 item_scores 时处理
            if not isinstance(item_scores, list):
                conn.rollback()
                return jsonify(
                    {"error": "评分数据格式错误：item_scores 必须是数组"}
                ), 400

            # a. 删除旧的评分记录
            cur.execute(
                "DELETE FROM evaluation_detail WHERE evaluation_id = %s",
                (evaluation_id,),
            )
            print(
                f"已删除旧的 evaluation_detail 记录 for evaluation_id: {evaluation_id}"
            )
            ...
            for item_score in item_scores:
                ...
                if item_id is None or score_raw is None:
                    print(
                        f"警告: item_scores 中缺少 item_id 或 score, 跳过: {item_score}"
                    )
                    continue
                ...
                if score_raw != "" and score_raw is not None:
                    try:
                        ...
                        if 0 <= score_val <= 100:
                            ...
                            print(
                                f"警告: score 值 {score_val} 超出范围 [0, 100]，将设为 NULL"
                            )
                    except (ValueError, TypeError):
                        print(
                            f"警告: 无法将 score '{score_raw}' 转换为整数，将设为 NULL"
                        )
                ...
                try:
                    ...
                    print(
                        f"插入 evaluation_detail 失败: eval_id={evaluation_id}, item_id={item_id}, score={score}, 错误: {insert_detail_error}"
                    )
                    conn.rollback()  # 单条插入失败也回滚
                    raise  # 重新抛出异常，让全局错误处理捕获

            print(
                f"已处理 {len(item_scores)} 条 item_scores for evaluation_id: {evaluation_id}"
            )

        # 3. 处理按类别的手动输入 (evaluation_manual_input)
        category_manual_inputs = data.get("category_manual_inputs")
        if (
            category_manual_inputs is not None
        ):  # 仅当提供了 category_manual_inputs 时处理
            if not isinstance(category_manual_inputs, dict):
                conn.rollback()
                return jsonify(
                    {"error": "手动输入数据格式错误：category_manual_inputs 必须是对象"}
                ), 400

            # a. 删除旧的手动输入记录
            cur.execute(
                "DELETE FROM evaluation_manual_input WHERE evaluation_id = %s",
                (evaluation_id,),
            )
            print(
                f"已删除旧的 evaluation_manual_input 记录 for evaluation_id: {evaluation_id}"
            )
            ...
            for category_id_str, manual_input_text in category_manual_inputs.items():
                if (
                    manual_input_text and str(manual_input_text).strip()
                ):  # 确保不为空或仅有空白
                    try:
                        ...
                        """,
                            (
                                evaluation_id,
                                category_id,
                                str(manual_input_text).strip(),
                            ),
                        )
                        manual_inputs_inserted += 1
                    except ValueError:
                        print(
                            f"警告: 无效的 category_id '{category_id_str}'，跳过手动输入。"
                        )
                    except Exception as insert_manual_error:
                        print(
                            f"插入 evaluation_manual_input 失败: eval_id={evaluation_id}, cat_id={category_id_str}, 错误: {insert_manual_error}"
                        )
                        conn.rollback()  # 单条插入失败也回滚
                        raise  # 重新抛出异常
            print(
                f"已处理 {manual_inputs_inserted} 条 category_manual_inputs for evaluation_id: {evaluation_id}"
            )
        ...
        if evaluation_record["evaluator_customer_id"] and "client_name" in data:
            ...
            """,
                (
                    ...
                    data.get("client_title", ""),
                    evaluation_record["evaluator_customer_id"],
                ),
            )
            print(
                f"已更新 customer 表 for customer_id: {evaluation_record['evaluator_customer_id']}"
            )

        conn.commit()
        print(f"评价更新成功，评价ID: {evaluation_id}")
        return jsonify(
            {"success": True, "message": "评价更新成功", "id": evaluation_id}
        )
