...
def get_cached_access_token(client):
    ...
    if wechat_cache["access_token"] and wechat_cache["access_token_expires_at"] > now:
        ...
        try:
            ...
            wechat_cache["access_token_expires_at"] = now + min(
                token_info.get("expires_in", 7200) - 200, CACHE_TTL
            )
            logger.info(
                f"获取到新的 access_token，有效期至: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(wechat_cache['access_token_expires_at']))}"
            )
            return wechat_cache["access_token"]
...
def get_cached_jsapi_ticket(client):
    ...
    if wechat_cache["jsapi_ticket"] and wechat_cache["jsapi_ticket_expires_at"] > now:
        ...
        try:
            ...
            # 设置过期时间，比微信给的 expires_in 稍短一点点
            wechat_cache["jsapi_ticket_expires_at"] = now + min(
                ticket_info.get("expires_in", 7200) - 200, CACHE_TTL
            )
            logger.info(
                f"获取到新的 jsapi_ticket，有效期至: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(wechat_cache['jsapi_ticket_expires_at']))}"
            )
            return wechat_cache["jsapi_ticket"]
        except WeChatException as e:
...
def get_jssdk_config():
    ...
    try:
        ...
        client = WeChatClient(
            WECHAT_APP_ID, WECHAT_APP_SECRET, session=None
        )  # 可以考虑使用 session 提高性能
        ...
        logger.debug(
            f"成功获取jsapi_ticket (可能来自缓存): {jsapi_ticket[:10]}..."
        )  # 只打印前10位
