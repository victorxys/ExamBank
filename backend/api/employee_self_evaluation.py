...
def submit_employee_self_evaluation():
    ...
    for evaluation in data["evaluations"]:
        if "item_id" not in evaluation or "score" not in evaluation:
            return jsonify(
                {"error": "Each evaluation must have item_id and score"}
            ), 400
...
@employee_self_evaluation_bp.route(
    "/api/employee-self-evaluation/<evaluation_id>", methods=["GET"]
)
# @jwt_required()
