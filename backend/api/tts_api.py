...
from backend.tasks import (
    merge_current_generated_audios_async,
)  # 导入新的Celery任务，合并当前已有的音频文件
    ...
    def wrapper(*args, **kwargs):
        if request.method == "OPTIONS":
            current_app.logger.debug(
                f"Admin_required: OPTIONS request for {request.path}, letting it pass for CORS."
            )
...
def create_training_content():
    ...
    if not all(field in data and data[field] for field in required_fields):
        return jsonify(
            {"error": "缺少必要字段: course_id, content_name, original_content"}
        ), 400
    ...
    try:
        ...
        try:
            ...
            llm_oral_prompt_uuid = (
                uuid.UUID(data.get("llm_oral_prompt_id"))
                if data.get("llm_oral_prompt_id")
                else None
            )
            llm_refine_prompt_uuid = (
                uuid.UUID(data.get("llm_refine_prompt_id"))
                if data.get("llm_refine_prompt_id")
                else None
            )
        except ValueError:
        ...
        if llm_refine_prompt_uuid and not LlmPrompt.query.get(
            str(llm_refine_prompt_uuid)
        ):
        ...
        new_content = TrainingContent(
            ...
            llm_oral_prompt_id=str(llm_oral_prompt_uuid)
            if llm_oral_prompt_uuid
            else None,
            llm_refine_prompt_id=str(llm_refine_prompt_uuid)
            if llm_refine_prompt_uuid
            else None,
        )
        ...
        current_app.logger.info(
            f"用户 {current_user_id_str} 创建了培训内容 {new_content.id} ({new_content.content_name})"
        )
...
def get_training_contents_by_course(course_id):
    ...
    try:
        ...
        contents = (
            TrainingContent.query.filter_by(course_id=str(course_id))
            ...
            [
                {
                    ...
                    "uploader_username": content.uploader.username
                    if content.uploader
                    else "N/A",
                }
        ...
        current_app.logger.error(
            f"获取课程 {course_id} 的培训内容失败: {e}", exc_info=True
        )
...
def get_training_content_detail(content_id):
    current_app.logger.info(
        f"--- get_training_content_detail CALLED for content_id: {content_id} ---"
    )  # 1. 确认函数被调用
    try:
        content = TrainingContent.query.get(str(content_id))
        if not content:
            current_app.logger.warning(
                f"TrainingContent with ID {content_id} not found."
            )
            return jsonify({"error": "培训内容未找到"}), 404
        # --- 关键的调试步骤：强制刷新对象 ---

        current_app.logger.info(
            f"获取到 TrainingContent 对象: {content.id}, 尝试从数据库刷新..."
        )
        ...
        current_app.logger.info(
            f"TrainingContent 对象 {content.id} 及其 tts_scripts 关系已处理。"
        )
        # --- 调试步骤结束 ---

        current_app.logger.info(
            f"Found TrainingContent: {content.id}, status: {content.status}"
        )  # 2. 确认内容找到

        scripts_data = []
        # 3. 仔细检查这里的循环和属性访问
        for script in content.tts_scripts.order_by(
            TtsScript.script_type, TtsScript.version.desc()
        ).all():
            ...
            script_info = {
                ...
                "content_preview": script.content[:200]
                + ("..." if len(script.content) > 200 else ""),
                "llm_call_log_id": str(script.llm_call_log_id)
                if script.llm_call_log_id
                else None,
                "source_script_id": str(script.source_script_id)
                if script.source_script_id
                else None,
            }
            scripts_data.append(script_info)

        current_app.logger.info(
            f"Processed {len(scripts_data)} scripts."
        )  # 4. 确认脚本处理数量

        sentences_data = []
        current_app.logger.info(
            f"--- 开始直接查询 TtsScript 表获取最新的 final_tts_script for content_id: {content.id} ---"
        )
                ...
                sentences_data.append(
                    {
                        ...
                        "latest_audio_url": latest_audio.file_path
                        if latest_audio
                        else None,
                        "latest_audio_id": str(latest_audio.id)
                        if latest_audio
                        else None,
                        "audio_duration_ms": latest_audio.duration_ms
                        if latest_audio
                        else None,
                        "tts_config": sentence.tts_config,  # 返回单句的保存配置
                        "generation_params": latest_audio.generation_params
                        if latest_audio
                        else None,
                        "tts_engine_used": latest_audio.tts_engine
                        if latest_audio
                        else None,
                        "model_used": model_used,  # 将提取出的模型信息传递给前端
                        "latest_audio_created_at": latest_audio.created_at.isoformat()
                        if latest_audio
                        else None,
                    }
                )
        ...
        current_app.logger.info(
            f"Processed {len(sentences_data)} sentences."
        )  # 6. 确认句子处理数量

        merged_audio_info = None
        # 注意：`content.merged_audios` 是一个 relationship，需要调用 .all() 或 .first()
        latest_merged_audio_query = content.merged_audios.filter_by(
        ...
        if latest_merged_audio:
            merged_audio_info = {
                ...
                "created_at": latest_merged_audio.created_at.isoformat()
                if latest_merged_audio.created_at
                else None,
            ...
            segments_for_merged = (
                MergedAudioSegment.query.filter_by(
                    merged_audio_id=latest_merged_audio.id
                )
                .order_by(MergedAudioSegment.original_order_index)
                .all()
            )
                ...
                merged_audio_info["segments"].append(
                    {
                        ...
                        "tts_sentence_id": str(seg.tts_sentence_id)
                        if seg.tts_sentence_id
                        else None,
        ...
        if latest_synthesis_task:
            synthesis_task_data = {
                ...
                "generated_resource_id": str(
                    latest_synthesis_task.generated_resource_id
                )
                if latest_synthesis_task.generated_resource_id
                else None,
        ...
        response_payload = {
            ...
            "original_content_preview": content.original_content[:500]
            + ("..." if len(content.original_content) > 500 else ""),
            "status": content.status,
            "created_at": content.created_at.isoformat()
            if content.created_at
            else None,
            ...
            "llm_oral_prompt_name": content.llm_oral_prompt.prompt_name
            if content.llm_oral_prompt
            else None,  # 8. 检查关联对象是否存在
            "llm_refine_prompt_name": content.llm_refine_prompt.prompt_name
            if content.llm_refine_prompt
            else None,  # 8. 检查关联对象是否存在
        ...
        current_app.logger.error(
            f"获取培训内容 {content_id} 详情失败: {e}", exc_info=True
        )  # 10. 捕获并记录详细错误
        return jsonify(
            {"error": "获取培训内容详情时发生服务器内部错误: " + str(e)}
        ), 500
...
def get_original_training_content(content_id):
    ...
    if not content:
        return jsonify({"error": "培训内容未找到"}), 404
    return jsonify(
        {"id": str(content.id), "original_content": content.original_content}
    )
...
def update_training_content(content_id):
    ...
    try:
        ...
        if "llm_refine_prompt_id" in data:
            if data["llm_refine_prompt_id"]:
                try:
                    ...
                    if not LlmPrompt.query.get(str(prompt_uuid)):
                        return jsonify(
                            {"error": "指定的修订refine脚本LLM Prompt不存在"}
                        ), 400
        ...
        db.session.commit()
        current_app.logger.info(
            f"培训内容 {content_id} 的 {', '.join(updated_fields)} 已更新"
        )
...
def delete_training_content(content_id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"培训内容 {content_id} ({content_name_for_log}) 已被删除"
        )
...
@admin_required
def update_script_content_route(
    script_id,
):  # Renamed to avoid conflict with get_script_content
    data = request.get_json()
    ...
    if "content" not in data:
        current_app.logger.warning(
            f"更新脚本 {script_id} 失败：请求中缺少 'content' 字段。"
        )
    ...
    try:
        ...
        if script_object.script_type == "final_tts_script":
            ...
            if script_object.training_content:  # 确保关联的 training_content 存在
                script_object.training_content.status = (
                    "final_script_edited_pending_resplit"
                )
                current_app.logger.info(
                    f"最终脚本 {script_object.id} (v{script_object.version}) 内容已更新。TrainingContent {script_object.training_content_id} 状态更新为 'final_script_edited_pending_resplit'。"
                )
            else:
                current_app.logger.warning(
                    f"最终脚本 {script_object.id} 没有关联的 training_content，无法更新其状态。"
                )

            db.session.add(
                script_object
            )  # 确保 script 对象的更改（包括 training_content 的状态，如果关系配置正确）被添加到会话
            db.session.commit()
            ...
            return jsonify(
                {
                    ...
                    "new_training_content_status": script_object.training_content.status
                    if script_object.training_content
                    else None,
                }
            )
        else:
            ...
            db.session.commit()
            current_app.logger.info(
                f"脚本 {script_object.id} ({script_object.script_type}) 内容已更新。"
            )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"更新脚本 {script_id} (原内容预览: {original_content_preview}...) 失败: {e}",
            exc_info=True,
        )
...
def update_sentence_text(sentence_id):
    ...
    if not isinstance(sentence_text_value, str):
        current_app.logger.error(
            f"更新句子 {sentence_id} 失败: sentence_text 期望是字符串，实际得到类型 {type(sentence_text_value)}，值为: {sentence_text_value}"
        )
        return jsonify(
            {
                "error": f"句子内容格式错误 (sentence_text should be a string, got {type(sentence_text_value).__name__})"
            }
        ), 400
    ...
    if not new_text_stripped:  # 检查 strip 之后是否为空
        return jsonify(
            {
                "error": "句子内容不能为空 (sentence_text cannot be empty after stripping)"
            }
        ), 400
    ...
    try:
        ...
        if original_text != new_text_stripped:
            ...
            current_app.logger.info(
                f"句子 {sentence.id} 被修改，标记为 'modified_after_merge=true'"
            )
            # ----------------------------->>>
            ...
            TtsAudio.query.filter_by(
                ...
                {"is_latest_for_sentence": False}, synchronize_session=False
            )  # 添加 synchronize_session=False

            db.session.commit()
            return jsonify(
                {
                    "message": "句子更新成功，语音状态已重置为待重新生成",
                    "id": str(sentence.id),
                }
            )  # 确保返回 str(uuid)
        else:
...
def get_audios_by_content(content_id):
    ...
    try:
            ...
            if file_path and not file_path.startswith(("http://", "https://")):
                ...
                return os.path.join(
                    current_app.config.get(
                        "TTS_AUDIO_BASE_URL_FOR_API", "/static/tts_audio"
                    ),
                    file_path,
                ).replace("\\", "/")  # 保证路径分隔符
            return file_path
            ...
            [
                {
                    ...
                    "sentence_id": str(audio.tts_sentence_id)
                    if audio.tts_sentence_id
                    else None,
                    ...
                    "is_latest": audio.is_latest_for_sentence
                    if audio.audio_type == "sentence_audio"
                    else audio.is_latest_for_content,
        ...
        current_app.logger.error(
            f"获取培训内容 {content_id} 的音频失败: {e}", exc_info=True
        )
...
def delete_audio_file_route(audio_id):
    ...
    try:
        ...
        db.session.commit()
        current_app.logger.info(
            f"TTS音频记录 {audio_id_for_log} 及其关联文件 {file_path_to_delete} (如果存在) 已被删除。"
        )
...
def _create_new_script_version(
...
def generate_oral_script_route(content_id):
    ...
    if content.llm_oral_prompt:
        ...
        current_app.logger.info(
            f"使用用户为内容 {content_id} 指定的口语化Prompt: {content.llm_oral_prompt.prompt_name}"
        )

    try:
        # 确保有原始脚本记录
        original_text_script = TtsScript.query.filter_by(
            training_content_id=content.id, script_type="original_text"
        ).first()
        ...
        current_app.logger.info(
            f"口播稿生成任务已提交 (Task ID: {task.id}) for content {content_id}"
        )
        return jsonify(
            {
        ...
        db.session.rollback()
        current_app.logger.error(
            f"提交口播稿生成任务失败 for content {content_id}: {e}", exc_info=True
        )
...
def tts_refine_script_route(oral_script_id):
    ...
    try:
        ...
        current_app.logger.info(
            f"TTS Refine 异步任务已启动 (Task ID: {task.id}) for oral_script {oral_script_id}"
        )
        return (
            jsonify(
                {
        ...
        db.session.commit()
        current_app.logger.error(
            f"提交 TTS Refine 任务失败 for oral_script {oral_script_id}: {e}",
            exc_info=True,
        )
        return jsonify(
            {"error": "提交 TTS Refine 任务时发生服务器错误: " + str(e)}
        ), 500
...
def llm_refine_script_route(refined_script_id):
    ...
    source_oral_script = (
        TtsScript.query.filter_by(
            training_content_id=training_content.id, script_type="oral_script"
        )
        .order_by(TtsScript.version.desc())
        .first()
    )
    ...
    if training_content.llm_refine_prompt:
        prompt_identifier_final_refine = (
            training_content.llm_refine_prompt.prompt_identifier
        )
        current_app.logger.info(
            f"使用用户为内容 {training_content.id} 指定的LLM最终修订Prompt: {training_content.llm_refine_prompt.prompt_name}"
        )
    ...
    try:
        ...
        current_app.logger.info(
            f"LLM最终修订任务已提交 (Task ID: {task.id}) for script {refined_script_id}"
        )
        return jsonify(
            {
        ...
        db.session.rollback()
        current_app.logger.error(
            f"提交LLM最终修订任务失败 for script {refined_script_id}: {e}",
            exc_info=True,
        )
...
@tts_bp.route(
    "/scripts/<uuid:source_final_script_id>/split-sentences", methods=["POST"]
)
@admin_required
def split_script_into_sentences_route(source_final_script_id):
    source_final_script_id_str = str(source_final_script_id)
    current_app.logger.info(
        f"--- [SplitTask Start] 源 final_script_id: {source_final_script_id_str} ---"
    )
    ...
    if not training_content:
        # ... (返回错误) ...
        current_app.logger.error(
            f"[SplitTask Fail] 源脚本 {source_final_script_id_str} 找不到关联的培训内容。"
        )
        return jsonify({"error": "找不到关联的培训内容"}), 404

    try:
        current_app.logger.info(
            f"[SplitTask] 开始处理源脚本 ID: {source_final_script.id}, Version: {source_final_script.version} 的内容拆分。"
        )

        current_training_content_id = (
            training_content.id
        )  # 提前获取，避免在循环中反复访问关系

        # 1. 确定新版本号
        current_max_version_obj = (
            db.session.query(func.max(TtsScript.version))
            .filter_by(
        ...
        current_max_version = (
            current_max_version_obj[0]
            if current_max_version_obj and current_max_version_obj[0] is not None
            else 0
        )
        new_version_number = current_max_version + 1
        current_app.logger.info(
            f"[SplitTask] 内容 {current_training_content_id} 的 final_tts_script 最大版本号为: {current_max_version}. 新版本将是: {new_version_number}"
        )
        ...
        current_app.logger.info(
            f"[SplitTask] 新的 final_tts_script (Version {new_version_number}) 已添加到 session。尝试 flush..."
        )

        try:
            db.session.flush()  # ⭐ 关键：尝试在这里 flush 以暴露早期问题
            current_app.logger.info(
                f"[SplitTask] Flush 成功。新脚本内存中 ID: {new_final_script.id} (如果数据库生成了)。"
            )
            if (
                not new_final_script.id
            ):  # 对于 UUID，SQLAlchemy 应该在 add 后就能填充，但 flush 是一个检查点
                current_app.logger.warning(
                    "[SplitTask] Flush 后 new_final_script 仍然没有 ID (这对于自增ID是正常的，但对于UUID可能意味着问题或延迟填充)。"
                )
        except Exception as e_flush_script:
            db.session.rollback()
            current_app.logger.error(
                f"[SplitTask Fail] Flush new_final_script 时出错: {e_flush_script}",
                exc_info=True,
            )
            return jsonify(
                {
                    "error": f"创建新脚本版本时内部错误 (flush script): {str(e_flush_script)}"
                }
            ), 500

        # 3. 拆分句子并尝试复用语音
        sentences_text_list = [
            s.strip() for s in new_final_script.content.split("\n") if s.strip()
        ]
        if not sentences_text_list:  # 尝试用句号分割作为备选
            sentences_text_list = [
                s.strip() + "。"
                for s in new_final_script.content.split("。")
                if s.strip()
            ]  # 保留句号
            if (
                not sentences_text_list and new_final_script.content.strip()
            ):  # 如果还有内容但无法分割，则视为一句
                sentences_text_list = [new_final_script.content.strip()]

        if not sentences_text_list:
            current_app.logger.warning(
                f"[SplitTask] 新脚本 {new_final_script.id if new_final_script.id else '未知ID'} 内容为空或无法拆分出任何句子。"
            )
        ...
        for index, new_sentence_text in enumerate(sentences_text_list):
            ...
            for old_script in historical_final_scripts_query.all():
                ...
                if matching_old_sentence:
                    latest_valid_audio_for_old_sentence = (
                        TtsAudio.query.filter(
                        ...
                        .filter(TtsSentence.audio_status == "generated")
                        .order_by(TtsAudio.created_at.desc())
                        .first()
                    )
            ...
            if found_audio_to_reuse:
                ...
                try:
                    ...
                    current_app.logger.debug(
                        f"[SplitTask] Sentence flushed, ID: {new_sentence.id}"
                    )
                except Exception as e_flush_sent:
                    current_app.logger.error(
                        f"[SplitTask] Flush new_sentence (reused audio) 时出错: {e_flush_sent}",
                        exc_info=True,
                    )
                    ...
                    new_sentence.audio_status = (
                        "error_creating_sentence_record"  # 标记一个错误状态
                    )
                    # continue # 或者跳过这个句子的语音记录创建

                if (
                    new_sentence.id and new_sentence.audio_status == "generated"
                ):  # 确保句子已成功 flush 且状态正确
                    reused_audio_entry = TtsAudio(
                        ...
                        generation_params={
                            "reused_from_audio_id": str(found_audio_to_reuse.id),
                    ...
                    current_app.logger.info(
                        f"[SplitTask] 句子 (新ID: {new_sentence.id}) '{new_sentence_text[:30]}...' 复用了历史语音 {found_audio_to_reuse.id}"
                    )
                elif (
                    new_sentence.audio_status != "error_creating_sentence_record"
                ):  # 如果句子创建失败，就不尝试关联语音了
                    new_sentence.audio_status = (
                        "pending_generation"  # 如果句子创建成功但语音关联失败
                    )
                    current_app.logger.warning(
                        f"[SplitTask] 句子 '{new_sentence_text[:30]}...' 未能成功关联复用语音，状态设为 pending_generation。"
                    )
        ...
        current_app.logger.info(
            "[SplitTask] 即将提交所有更改到数据库 (新脚本版本、句子、复用语音记录、内容状态)..."
        )
        db.session.commit()  # ⭐ 最终提交
        current_app.logger.info("[SplitTask] 数据库提交成功。")

        # 在 commit 后再次确认新脚本是否真的写入了
        persisted_script = TtsScript.query.get(
            new_final_script.id
            if new_final_script.id
            else "00000000-0000-0000-0000-000000000000"
        )  # 用一个无效UUID避免None错误
        if persisted_script:
            current_app.logger.info(
                f"[SplitTask] 确认：新脚本 ID {persisted_script.id} (Version {persisted_script.version}) 已成功持久化到数据库。"
            )
        else:
            current_app.logger.error(
                f"[SplitTask Fail] 严重错误：Commit后，新脚本 ID {new_final_script.id if new_final_script.id else '未知ID'} 在数据库中未找到！事务可能未生效！"
            )
        ...
        return jsonify(
            {
                ...
                "new_final_tts_script_id": str(new_final_script.id)
                if new_final_script.id
                else None,
        ...
        db.session.rollback()
        current_app.logger.error(
            f"[SplitTask Fail] 在主 try 块中捕获到异常，已回滚: {e}", exc_info=True
        )
        return jsonify(
            {"error": "重新拆分脚本并创建新版本时发生服务器错误: " + str(e)}
        ), 500
...
def get_task_status(task_id):
    ...
    if task.status == "SUCCESS":
        ...
        if isinstance(task.result, dict):
            ...
            final_counts = {
                "message": task.result.get(
                    "message", response_data["meta"].get("message", "任务成功完成")
                ),
                "total_in_batch": task.result.get(
                    "total_in_batch",
                    task.result.get(
                        "total", response_data["meta"].get("total_in_batch", 0)
                    ),
                ),
                "processed_in_batch": task.result.get(
                    "processed_in_batch",
                    response_data["meta"].get(
                        "processed_in_batch",
                        (
                            task.result.get("succeeded", 0)
                            + task.result.get("failed", 0)
                        ),
                    ),
                ),
                "succeeded_in_batch": task.result.get(
                    "succeeded_in_batch",
                    task.result.get(
                        "succeeded", response_data["meta"].get("succeeded_in_batch", 0)
                    ),
                ),
                "failed_in_batch": task.result.get(
                    "failed_in_batch",
                    task.result.get(
                        "failed", response_data["meta"].get("failed_in_batch", 0)
                    ),
                ),
            }
            ...
            if task.result.get("status") in ["Error", "FAILURE"]:
                ...
                response_data["error_message"] = task.result.get(
                    "message", "任务执行返回错误状态"
                )
        elif (
            "message" not in response_data["meta"]
        ):  # 如果 result 不是字典，但 meta 中没有消息
    ...
    # 如果这些状态下 task_info_dict 为空，或者缺少 message，再进行补充
        ...
        if task.status == "PROGRESS":
            total = response_data["meta"].get(
                "total_in_batch", response_data["meta"].get("total", 0)
            )
            processed = response_data["meta"].get(
                "processed_in_batch", response_data["meta"].get("current", 0)
            )
            if total > 0:
    ...
    default_keys_for_meta = {
        ...
        "message": "状态加载中...",
        "last_processed_sentence_id": None,
        "last_processed_sentence_status": None,
...
def generate_sentence_audio_route(sentence_id):
    ...
    try:
        ...
        tts_engine_to_use = request_data.get(
            "tts_engine", "gradio_default"
        )  # 前端可以传 "gemini_tts"
        current_app.logger.info(
            f"API generate_sentence_audio_route received tts_engine: {tts_engine_to_use} for sentence {sentence_id}"
        )
        # 获取特定于引擎的参数
        ...
        task = generate_single_sentence_audio_async.delay(
            ...
            pt_file_path_relative=pt_file_to_use_for_gradio
            if tts_engine_to_use == "gradio_default"
            else None,  # Gemini TTS 不需要本地 PT 文件
            tts_engine_params=tts_engine_params_from_request,
            override_config=override_config,
        )

        current_app.logger.info(
            f"单句语音生成异步任务已启动 (Task ID: {task.id}, Engine: {tts_engine_to_use}) for sentence {sentence_id}"
        )
        return jsonify(
            {
        ...
        if sentence:  # 如果在提交任务前出错，回滚状态
            ...
            db.session.commit()
        current_app.logger.error(
            f"提交单句语音生成任务失败 for sentence {sentence_id}: {e}", exc_info=True
        )
        return jsonify(
            {"error": "提交单句语音生成任务时发生服务器错误: " + str(e)}
        ), 500


@tts_bp.route(
    "/training-contents/<uuid:content_id>/batch-generate-audio", methods=["POST"]
)
@admin_required
def batch_generate_audio_for_content_route(content_id):
    current_app.logger.info(
        f"API batch_generate_audio_for_content_route received content_id: {content_id}"
    )
    ...
    if not latest_final_script:
        current_app.logger.error(
            f"API: No final_tts_script found for content {content.id} via DIRECT QUERY."
        )
        return jsonify(
            {"error": "未找到最终TTS脚本（直接查询失败），无法进行批量语音生成。"}
        ), 404
    current_app.logger.info(
        f"API: Found latest final_tts_script via ==========>DIRECT QUERY with ID: {latest_final_script.id}, Version: {latest_final_script.version}"
    )

    # latest_final_script = content.tts_scripts.filter_by(script_type='final_tts_script').order_by(TtsScript.version.desc()).first()

    current_app.logger.info(
        f"API: Found latest final_tts_script with ID+>+>+>+>+>+>+>+>+>+>+>: {latest_final_script.id}, Version: {latest_final_script.version}"
    )
    ...
    try:
        ...
        pt_file_for_gradio = request_data.get(
            "pt_file_path_relative",
            current_app.config.get("DEFAULT_GRADIO_PT_FILE_PATH"),
        )  # 使用 current_app
        ...
        current_app.logger.info(
            f"==========latest_final_script======={str(latest_final_script.id)}"
        )
        task = batch_generate_audio_task.delay(  # 假设任务变量名是这个
            final_script_id_str=str(latest_final_script.id),
            tts_engine_identifier=tts_engine_to_use,
            pt_file_path_relative_for_gradio=pt_file_for_gradio
            if tts_engine_to_use == "gradio_default"
            else None,  # <--- 修改这里
        ...
        current_app.logger.info(
            f"批量语音生成异步任务已启动 (Task ID: {task.id}, Engine: {tts_engine_to_use}) for content {content_id}"
        )
        return jsonify(
            {
        ...
        db.session.rollback()
        current_app.logger.error(
            f"提交批量语音生成任务失败 (内容ID: {content_id}): {e}", exc_info=True
        )
...
def delete_sentence_route(sentence_id):
    ...
    try:
        # 1. 找到所有关联的语音文件记录
        audios_to_delete = TtsAudio.query.filter_by(
            tts_sentence_id=sentence_id_str
        ).all()
        ...
        if audio_paths_to_delete_physically:
            current_app.logger.warning(
                f"TODO: 实现物理删除以下文件: {audio_paths_to_delete_physically}"
            )

        current_app.logger.info(
            f"句子 {sentence_id_str} 及其数据库中的语音记录已删除。"
        )
        return jsonify(
            {"message": "句子及其关联语音记录已成功删除。请确保物理文件也已处理。"}
        )
    except Exception as e:
        db.session.rollback()
...
def update_training_content_original_text(content_id):
    ...
    try:
        if content.original_content != new_original_content:
            ...
            db.session.commit()
            current_app.logger.info(
                f"培训内容 {content_id} 的原始文本已更新。状态重置为 {content.status}。"
            )
            return jsonify(
                {
                    "message": "原始培训内容更新成功，后续处理流程可能需要重新执行。",
                    "id": str(content.id),
                }
            )
        else:
            return jsonify(
                {"message": "原始培训内容未发生改变。", "id": str(content.id)}
            ), 200

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"更新培训内容 {content_id} 的原始文本失败: {e}", exc_info=True
        )
...
def merge_content_audio_route(content_id):
    ...
    if (
        not latest_final_script.tts_sentences.first()
    ):  # Check if script has any sentences
    ...
    try:
        ...
        current_app.logger.info(
            f"语音合并任务已提交 (Task ID: {task.id}) for TrainingContent {content_id}"
        )
        return jsonify(
            {
        ...
        # db.session.commit()
        current_app.logger.error(
            f"提交语音合并任务失败 for content {content_id}: {e}", exc_info=True
        )
...
def get_merged_audio_segments_route(merged_audio_id):
    ...
    segments = (
        MergedAudioSegment.query.filter_by(merged_audio_id=merged_audio_record.id)
        .order_by(MergedAudioSegment.original_order_index)
        .all()
    )
        ...
        [
            {
                ...
                "end_ms": seg.end_ms,
                "duration_ms": seg.duration_ms,
                "sentence_current_text": seg.tts_sentence.sentence_text
                if seg.tts_sentence
                else None,  # Get current text if sentence still exists
            }
            for seg in segments
        ]
    )
...
def get_latest_synthesis_task_for_content(content_id):
    ...
    try:
            ...
            jsonify(
                {
                    ...
                    "generated_resource_id": str(latest_task.generated_resource_id)
                    if latest_task.generated_resource_id
                    else None,
        ...
        current_app.logger.error(
            f"获取最新合成任务失败 for content {content_id}: {e}", exc_info=True
        )
...
def start_video_synthesis_analysis(content_id):
    ...
    latest_merged_audio = (
        content.merged_audios.filter_by(is_latest_for_content=True)
        .order_by(TtsAudio.created_at.desc())
        .first()
    )
    ...
    if not os.path.exists(srt_txt_full_path):
        ...
        return jsonify(
            {
                "error": f"找不到对应的字幕文件({srt_txt_path_relative})，请确认第五步已成功生成。"
            }
        ), 404
    ...
    video_synthesis_dir = os.path.join(
        current_app.instance_path, "uploads", "video_synthesis", str(content_id)
    )
    ...
    try:
        ...
        current_app.logger.info(
            f"已触发视频脚本分析的异步任务，Celery Task ID: {async_task.id}"
        )
...
def trigger_video_synthesis(synthesis_id):
    ...
    if synthesis_task.status != "analysis_complete":
        return jsonify(
            {
                "error": f'任务当前状态为 "{synthesis_task.status}"，无法开始合成。请先完成分析。'
            }
        ), 400
    ...
    try:
        ...
        current_app.logger.info(
            f"已触发视频合成异步任务，Celery Task ID: {async_task.id}"
        )
...
def reset_synthesis_task(synthesis_id):
    ...
    try:
        ...
        if synthesis_task.generated_resource_id:
            ...
            current_app.logger.info(
                f"任务 {synthesis_id} 与旧资源 {old_resource_id} 的关联已解除。"
            )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"重置合成任务 {synthesis_id} 时出错: {e}", exc_info=True
        )
...
def update_video_script(synthesis_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(
            f"更新视频脚本 {synthesis_id} 时出错: {e}", exc_info=True
        )
        return jsonify({"error": "更新视频脚本时发生服务器内部错误"}), 500


@tts_bp.route(
    "/training-contents/<uuid:content_id>/merge-current-audio", methods=["POST"]
)
@admin_required
def merge_current_audio_route(content_id):
    ...
    try:
        ...
        current_app.logger.info(
            f"合并当前已生成语音任务已启动 (Task ID: {task.id}) for content {content_id}"
        )
        return jsonify(
            {
        ...
        if content:
            ...
            db.session.commit()
        current_app.logger.error(
            f"提交合并当前语音任务失败 (内容ID: {content_id}): {e}", exc_info=True
        )
...
def update_training_content_tts_config(content_id):
    ...
    try:
        ...
        return jsonify(
            {"message": "全局TTS配置保存成功", "config": content.default_tts_config}
        )
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"更新全局TTS配置失败 for content {content_id}: {e}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def update_sentence_tts_config(sentence_id):
    ...
    try:
        ...
        return jsonify(
            {"message": "单句TTS配置保存成功", "config": sentence.tts_config}
        )
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(
            f"更新单句TTS配置失败 for sentence {sentence_id}: {e}", exc_info=True
        )
        return jsonify({"error": str(e)}), 500
...
def trigger_resplit_and_match_route(final_script_id):
    ...
    if not script or script.script_type != "final_tts_script":
        current_app.logger.warning(
            f"手动触发重新拆分失败：脚本 {final_script_id} 不是有效的最终脚本。"
        )
        return jsonify({"error": "无效的最终脚本ID或类型不匹配"}), 400

    if not script.training_content:
        current_app.logger.warning(
            f"手动触发重新拆分失败：脚本 {final_script_id} 未关联到培训内容。"
        )
    ...
    try:
        ...
        current_app.logger.info(
            f"手动触发：为最终脚本 {script.id} (v{script.version}) 提交重新拆分和匹配句子任务 (ID: {task.id})。"
        )
        return jsonify(
            {
        ...
        if script.training_content:
            ...
            db.session.commit()
        current_app.logger.error(
            f"手动触发展开和匹配任务失败 for script {final_script_id}: {e}",
            exc_info=True,
        )
...
def skip_tts_refine_route(oral_script_id):
    ...
    if not oral_script or oral_script.script_type != "oral_script":
        current_app.logger.warning(
            f"跳过TTS Refine失败：口播脚本 {oral_script_id} 未找到或类型不正确。"
        )
        return jsonify({"error": "无效的口播脚本ID或类型不匹配"}), 400
    ...
    if not training_content:
        current_app.logger.warning(
            f"跳过TTS Refine失败：口播脚本 {oral_script_id} 未关联培训内容。"
        )
        return jsonify({"error": "脚本未关联到培训内容"}), 404

    try:
        current_app.logger.info(
            f"用户请求跳过TTS Refine步骤，将口播脚本 {oral_script.id} (v{oral_script.version}) 内容直接作为 TTS Refine稿。"
        )
        ...
        current_app.logger.info(
            f"TTS Refine步骤已跳过。新的TTS Refine稿 (ID: {new_tts_refined_script.id}, v{new_tts_refined_script.version}) 已创建，内容复制自口播稿 {oral_script.id}。TrainingContent {training_content.id} 状态更新为 'pending_llm_final_refine'。"
        )
        ...
        db.session.rollback()
        current_app.logger.error(
            f"跳过TTS Refine步骤失败 for oral_script {oral_script_id}: {e}",
            exc_info=True,
        )
