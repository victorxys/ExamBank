...
bp = Blueprint('evaluation_item_api', __name__, url_prefix='/api/evaluation_item') # 修改蓝图名称
...
def create_evaluation_item():
    ...
    if not data or 'item_name' not in data or not data['item_name'].strip() or 'category_id' not in data:
    ...
    try:
        ...
        cur.execute("SELECT id FROM evaluation_category WHERE id = %s", (data['category_id'],))
        if not cur.fetchone():
...
def delete_evaluation_item(item_id):
    ...
    try:
        # 检查是否有评价详情或自评详情依赖
        cur.execute("SELECT COUNT(*) FROM evaluation_detail WHERE item_id = %s", (item_id,))
        eval_detail_count = cur.fetchone()[0]
        cur.execute("SELECT COUNT(*) FROM employee_self_evaluation_detail WHERE item_id = %s", (item_id,))
        ...
        cur.execute("DELETE FROM evaluation_item WHERE id = %s RETURNING id", (item_id,))
