...
def get_user_permissions(user_id_str):
    ...
    granted_course_ids = [
        str(uca.course_id)
        for uca in UserCourseAccess.query.filter_by(user_id=user_id).all()
    ]
    ...
    if expiry_type == "permanent":
        ...
        try:
            ...
            current_app.logger.warning(
                f"Invalid custom_expiry_date format: {custom_date_str}"
            )
            return None  # 或者抛出错误
    return None  # 默认或错误情况
...
def update_user_permissions(user_id_str):
    ...
    new_resource_permissions_map = {
        ...
        for item in resource_permissions_data
    ...
    try:
        # 1. 处理课程权限 (保持不变)
        UserCourseAccess.query.filter_by(user_id=user_id).filter(
            UserCourseAccess.course_id.notin_(new_granted_course_ids)
        ).delete(synchronize_session=False)
        current_course_permissions = {
            str(uca.course_id)
            for uca in UserCourseAccess.query.filter_by(user_id=user_id).all()
        }
        for course_id_to_add in new_granted_course_ids:
            if course_id_to_add not in current_course_permissions:
                if not TrainingCourse.query.get(course_id_to_add):
                    current_app.logger.warning(
                        f"Attempted to grant permission for non-existent course_id: {course_id_to_add}"
                    )
                    continue
                db.session.add(
                    UserCourseAccess(user_id=user_id, course_id=course_id_to_add)
                )

        # 2. 处理资源权限 (重点修改)
        # 删除不再授予的资源权限
        UserResourceAccess.query.filter_by(user_id=user_id).filter(
            UserResourceAccess.resource_id.notin_(new_granted_resource_ids)
        ).delete(synchronize_session=False)

        current_resource_access_records = UserResourceAccess.query.filter_by(
            user_id=user_id
        ).all()
        current_resource_permissions_map = {
            str(ura.resource_id): ura for ura in current_resource_access_records
        }

        for resource_id_str, perm_info in new_resource_permissions_map.items():
            expires_at_val = calculate_expiry_date(
                perm_info["expiry_type"], perm_info.get("custom_expiry_date")
            )
            ...
            if resource_id_str in current_resource_permissions_map:
                ...
                if existing_record.expires_at != expires_at_val:  # 仅当有效期改变时更新
                    ...
                    current_app.logger.info(
                        f"Updating expiry for resource {resource_id_str} for user {user_id} to {expires_at_val}"
                    )
            else:
                # 添加新的资源权限
                ...
                if not resource_obj:
                    current_app.logger.warning(
                        f"Attempted to grant permission for non-existent resource_id: {resource_id_str}"
                    )
                    continue
                ...
                current_app.logger.info(
                    f"Adding new access for resource {resource_id_str} for user {user_id}, expires_at: {expires_at_val}"
                )
        ...
        current_app.logger.error(
            f"Error updating permissions for user {user_id}: {e}", exc_info=True
        )
    ...
    try:
        ...
        for course in courses:
            ...
            resources = (
                ...
                .order_by(CourseResource.sort_order, CourseResource.name)
                .all()
            )
        ...
        current_app.logger.error(
            f"Error fetching courses with resources: {e}", exc_info=True
        )
        return jsonify(
            {"error": f"Failed to fetch courses with resources: {str(e)}"}
        ), 500
