...
def get_llm_call_logs():
    ...
    if function_name_filter:
        query = query.filter(
            LlmCallLog.function_name.ilike(f"%{function_name_filter}%")
        )
    if status_filter:
    ...
    if prompt_id_filter:
        try:
            query = query.filter(
                LlmCallLog.llm_prompt_id == uuid.UUID(prompt_id_filter)
            )
        except ValueError:
    ...
    try:
        logs_pagination = query.order_by(LlmCallLog.timestamp.desc()).paginate(
                ...
                "items": [
                    {
                        ...
                        "model_name": log.llm_model_log_ref.model_name
                        if log.llm_model_log_ref
                        else "N/A",
                        "prompt_name": log.llm_prompt_log_ref.prompt_name
                        if log.llm_prompt_log_ref
                        else "N/A",
                        "prompt_version": log.llm_prompt_log_ref.version
                        if log.llm_prompt_log_ref
                        else "N/A",
                        "api_key_name": log.api_key_name,
                        "status": log.status,
                        "duration_ms": log.duration_ms,
                        "user_username": log.user_ref.username
                        if log.user_ref
                        else "N/A",
                    }
                    for log in logs
        ...
        return jsonify(
            {
                ...
                "model_identifier": log.llm_model_log_ref.model_identifier
                if log.llm_model_log_ref
                else "N/A",
                "prompt_name": log.llm_prompt_log_ref.prompt_name
                if log.llm_prompt_log_ref
                else "N/A",
                "prompt_identifier": log.llm_prompt_log_ref.prompt_identifier
                if log.llm_prompt_log_ref
                else "N/A",
                "prompt_version": log.llm_prompt_log_ref.version
                if log.llm_prompt_log_ref
                else "N/A",
                "prompt_template": log.llm_prompt_log_ref.prompt_template
                if log.llm_prompt_log_ref
                else "N/A",  # 包含模板内容
                "api_key_name": log.api_key_name,
