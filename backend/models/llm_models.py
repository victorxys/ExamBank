...
class LlmModel(db.Model):
    ...
    provider = db.Column(db.String, nullable=False) # e.g., "Google Gemini", "OpenAI"
...
class LlmApiKey(db.Model):
    ...
    provider = db.Column(db.String, nullable=False) # Should match a provider in LlmModel
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    description = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class LlmPrompt(db.Model):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    prompt_name = db.Column(db.String, unique=True, nullable=False) # Business identifier
    system_instruction = db.Column(db.Text, nullable=False)
    description = db.Column(db.Text, nullable=True)
    model_id = db.Column(UUID(as_uuid=True), db.ForeignKey("llm_models.id"), nullable=True)
...
class LlmLog(db.Model):
    ...
    input_data = db.Column(JSONB, nullable=True) # Or db.Text if JSONB is not always suitable
    ...
    # ForeignKey to your User model. Adjust "user.id" if your table/column names are different.
    ...
    def __repr__(self):
        return f"<LlmLog {self.id} - {self.calling_function} - {'Success' if self.success else 'Failure'}>"