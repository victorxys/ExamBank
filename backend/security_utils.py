...
def get_fernet_instance_for_script():
    ...
    if not encryption_key_str:
        logger.error(
            f"错误: 环境变量 '{ENCRYPTION_KEY_ENV_VAR}' 未设置。请在 .env 文件中设置它。"
        )
        logger.info(
            "你可以使用以下Python代码生成一个新的密钥: from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"
        )
    ...
    try:
        ...
        logger.error(
            f"错误：无法使用环境变量 '{ENCRYPTION_KEY_ENV_VAR}' 初始化加密器。请确保其为有效的Fernet密钥。错误: {e}",
            exc_info=True,
        )
        raise
    ...
    if _fernet_instance is None:
        ...
        if not encryption_key_str:
            effective_logger.error(
                f"关键错误: 环境变量 '{ENCRYPTION_KEY_ENV_VAR}' 未设置。请在 .env 文件中设置它。"
            )
            ...
            raise RuntimeError(
                f"关键配置缺失: 环境变量 '{ENCRYPTION_KEY_ENV_VAR}' 未设置。应用无法安全运行。"
            )
...
def decrypt_data(encrypted_data: str, use_script_fernet=False) -> str:
    ...
    if not isinstance(encrypted_data, str):
        ...
        effective_logger.warning(
            "解密数据类型错误，期望字符串，得到: %s", type(encrypted_data)
        )
    ...
    try:
        ...
        effective_logger = current_app.logger if current_app else logger
        effective_logger.warning(
            f"警告: 解密数据失败。可能原因：密钥不匹配或数据已损坏。返回空字符串。错误详情: {e}"
        )
