...
# -----------------------
from backend.api.temp_answer import (
    save_temp_answer,
    get_temp_answers,
    mark_temp_answers_submitted,
)  # 使用绝对导入
from backend.api.evaluation import (
    get_evaluation_items,
...
from backend.api.evaluation_item import (
    bp as evaluation_item_api_bp,
)  # 保留或确认蓝图名称未冲突
...
app.config["TTS_AUDIO_STORAGE_PATH"] = os.path.join(
    app.root_path, "static", "tts_audio"
)
...
app.config["JWT_TOKEN_LOCATION"] = [
    "headers",
    "cookies",
]  # 允许从请求头和 Cookie 中获取 Token
app.config["JWT_COOKIE_SECURE"] = (
    False  # 开发环境可以设为 False，生产环境应为 True (HTTPS)
)
...
def expired_token_response(jwt_header, jwt_payload):
    response = jsonify(
        {"status": 401, "sub_status": 44, "msg": "令牌已过期 (Token has expired)"}
    )
    response.status_code = 401
    return add_cors_headers_to_response(response)
...
def token_not_fresh_response(jwt_header, jwt_payload):
    response = jsonify(
        {"status": 401, "sub_status": 46, "msg": "需要新的令牌 (Fresh token required)"}
    )
    response.status_code = 401
    return add_cors_headers_to_response(response)
...
AVATAR_DATA_FOLDER = os.path.join(
...
def login():
    ...
    try:
        cur.execute(
            'SELECT * FROM "user" WHERE "phone_number" = %s', (data["phone_number"],)
        )
        user = cur.fetchone()
...
def login_api():
    ...
    try:
        ...
        # 查询 status 列
        cur.execute(
            'SELECT id, username, password, role, status FROM "user" WHERE "phone_number" = %s',
            (phone_number,),
        )
        user = cur.fetchone()
        log.debug(
            f"User lookup completed for phone: {phone_number}. User found: {bool(user)}"
        )

        # --- 检查逻辑保持不变 ---
        if user is None:
            log.warning(
                f"API login failed: Phone number {phone_number} not found in database."
            )
            # 注意：因为没有连接池，不需要在这里 return 后关闭连接，finally会处理
        elif user["status"] != "active":
            log.warning(
                f"API login failed for phone number: {phone_number}. User status is '{user['status']}' (not active)."
            )
            # 同上，finally 会处理关闭
        elif check_password_hash(user["password"], password):
            log.info(
                f"API login successful for user ID: {user['id']} (Phone: {phone_number})"
            )
            access_token = create_token_with_role(user["id"], user["role"])
            # 成功时，在 finally 关闭前返回
            return jsonify(
            ...
            # 密码错误
            log.warning(
                f"API login failed for phone number: {phone_number} (Incorrect password)"
            )
            # 同上，finally 会处理关闭
        ...
        if conn:
            conn.close()
        log.debug(
            f"Database connection closed for API login attempt (phone: {phone_number})"
        )
...
def get_courses():
    ...
    try:
        ...
        if not is_admin:
            ...
            user_course_ids_query = (
                UserCourseAccess.query.with_entities(UserCourseAccess.course_id)
                .filter_by(user_id=current_user_id)
                .all()
            )
            # allowed_course_ids = [str(row[0]) for row in user_course_ids_query]
            allowed_course_ids = [
                row[0] for row in user_course_ids_query if row[0] is not None
            ]  # <<<--- 直接使用 UUID 对象，并过滤 None (如果有的话)
...
def check_course_deleteable(course_id):
    ...
    try:
        # 检查是否有关联的知识点
        cur.execute(
            "SELECT COUNT(*) FROM knowledgepoint WHERE course_id = %s", (course_id,)
        )
        knowledge_point_count = cur.fetchone()["count"]

        if knowledge_point_count > 0:
            return jsonify(
                {"deleteable": False, "message": "该课程包含知识点，无法删除"}
            )
...
def delete_course(course_id):
    ...
    try:
        ...
        # 删除课程
        cur.execute(
            "DELETE FROM trainingcourse WHERE id = %s RETURNING id", (course_id,)
        )
...
def create_knowledge_point():
    ...
    try:
        ...
        """,
            (data["point_name"], data["course_id"], data.get("description", "")),
        )
        point = cur.fetchone()
        # print("创建知识点结果：", point)
...
def delete_knowledge_point(point_id):
    ...
    try:
        # 检查是否有关联的考题
        cur.execute(
            "SELECT COUNT(*) FROM question WHERE knowledge_point_id = %s", (point_id,)
        )
        ...
        # 删除知识点
        cur.execute(
            "DELETE FROM knowledgepoint WHERE id = %s RETURNING id", (point_id,)
        )
...
def check_question_usage(question_id):
    ...
    try:
        ...
        result = cur.fetchone()
        return jsonify({"is_used_in_exam": result["is_used"] if result else False})
    except Exception as e:
...
def get_evaluation_structure():
    ...
    try:
        ...
        filter_by_visibility = (
            client_visible_param and client_visible_param.lower() == "true"
        )
        ...
        category_map = {
            str(cat["id"]): {
                **cat,
                "allow_manual_input": cat["allow_manual_input"],
                "items": [],
            }
        ...
        for category_id, category_items in item_map.items():
            if category_id in category_map:
                category_map[category_id]["items"] = sorted(
                    category_items, key=lambda x: x.get("sort_order", 0)
                )
        ...
        for aspect in aspects:
            aspect_data = {
                ...
                "type": "aspect",
                "sort_order": aspect.get("sort_order", 0),
                ...
                if str(category["aspect_id"]) == str(aspect["id"]):
                    ...
                    if processed_category:
                        category_data = {
                            ...
                            "allow_manual_input": processed_category[
                                "allow_manual_input"
                            ],  # *** 添加 allow_manual_input ***
            ...
            aspect_data["children"] = sorted(
                aspect_categories_sorted, key=lambda x: x.get("sort_order", 0)
            )
    ...
    try:
        ...
        for record in records:
            exam_paper = {
                ...
                "course_names": [
                    course for course in record["course_names"] if course is not None
                ]
                if record["course_names"]
                else [],
            }
    ...
    try:
        ...
        if single_count > 0:
            ...
            if available_single < single_count:
                conn.rollback()
                return jsonify(
                    {"error": f"单选题数量不足，只有 {available_single} 道题可用"}
                ), 400
        ...
        if multiple_count > 0:
            ...
            if available_multiple < multiple_count:
                conn.rollback()
                return jsonify(
                    {"error": f"多选题数量不足，只有 {available_multiple} 道题可用"}
                ), 400
...
def get_exam_for_taking(exam_id):
    ...
    try:
        ...
        # print(questions)
        # 按题型分组
        grouped_questions = {"single": [], "multiple": []}
        ...
        return jsonify({"exam": exam, "questions": grouped_questions})
...
def submit_exam_answer(exam_id):
    ...
    if user_id_from_request:
        try:
            ...
            logger.warning(
                f"提交考试时，请求中的user_id '{user_id_from_request}' 格式无效。"
            )
    try:
        # Input validation
        if not exam_id:
        ...
        start_time = (
            ...
            if start_time_record and start_time_record["start_time"]
        ...
        for answer in answers:
            ...
            if question_info["question_type"] == "单选题":
                ...
                is_correct = (
                    ...
                    and selected_option_str in correct_option_ids
                )
                ...
                if isinstance(question_info["correct_option_ids"], str):
                    correct_options = (
                        question_info["correct_option_ids"].strip("{}").split(",")
                    )
                    correct_options = [
                        str(uuid.UUID(opt.strip()))
            ...
            try:
                ...
                kp_coreect = {
                    "knowledge_point_name": question_info.get(
                        "knowledge_point_name", "未知知识点"
                    ),
                    "if_get": "已掌握"
                    if is_correct
                    else "未掌握",  # Python 的三元条件表达式
                }
    ...
    try:
        ...
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "DELETE FROM answerrecord WHERE exam_paper_id = %s", (exam_id,)
                )
...
def get_evaluation_detail(evaluation_id):
    ...
    try:
        ...
        manual_inputs_for_category = {
            str(row["category_id"]): row["manual_input"] for row in manual_inputs_raw
        }
        ...
        for row in structure_rows:
            ...
            if category_id not in final_aspects_map[aspect_id]["categories"]:
                ...
                valid_scores = [
                    item["average_score"] for item in items_for_this_category
                ]
                avg_score = (
                ...
                final_aspects_map[aspect_id]["categories"][category_id] = {
                    ...
                    "manual_input": manual_inputs_for_category.get(
                        category_id
                    ),  # Assign manual input
        ...
        for aspect_id, aspect_data in final_aspects_map.items():
            ...
            categories_list = sorted(
                list(aspect_data["categories"].values()),
            ...
            aspect_data["average_score"] = (
                sum(all_aspect_scores) / len(all_aspect_scores)
                if all_aspect_scores
                else 0.0
            )
        ...
        result = {
            ...
            "aspects": sorted(
                aspects_list, key=lambda x: x.get("sort_order", 0)
            )  # Add the processed aspects list
        }
...
def create_evaluation_route():
    ...
    if (
        not data
        or "evaluated_user_id" not in data
        or ("item_scores" not in data and "category_manual_inputs" not in data)
    ):  # *** 使用 category_manual_inputs ***
    ...
    category_manual_inputs = data.get(
        "category_manual_inputs", {}
    )  # *** 获取 category 手动输入 ***
    ...
    try:
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                ...
                if evaluation_type == "internal":
                    ...
                    if not evaluator_user_id:
                        print(
                            "Validation Error: Missing evaluator_user_id for internal evaluation."
                        )
                    ...
                    if not client_name:
                        print(
                            "Validation Error: Missing client_name for client evaluation."
                        )
                    ...
                    print(
                        f"Client evaluation, created/found customer: {evaluator_customer_id}"
                    )
                else:
                    print(
                        f"Validation Error: Invalid evaluation_type: {evaluation_type}"
                    )
                ...
                if item_scores:
                    ...
                    for score_data in item_scores:
                        ...
                        if (
                            ...
                            try:
                                ...
                                print(
                                    f"  Adding item score: item_id={item_id}, score={score_int}"
                                )
                                values_str_detail.append("(%s, %s, %s)")
                                params_detail.extend(
                                    [evaluation_id, item_id, score_int]
                                )
                            except (ValueError, TypeError):
                                print(
                                    f"  Skipping invalid score for item {item_id}: {score_val}"
                                )
                                continue
                        else:
                    ...
                    if params_detail:
                        sql_detail = (
                            "INSERT INTO evaluation_detail (evaluation_id, item_id, score) VALUES "
                            + ", ".join(values_str_detail)
                        )
                        # *** 打印最终的 SQL 和 参数 ***
                ...
                if category_manual_inputs:  # *** 检查 category_manual_inputs ***
                    ...
                    cur.execute(
                        "SELECT id FROM evaluation_category WHERE allow_manual_input = TRUE"
                    )
                    ...
                    for (
                        category_id,
                        input_data,
                    ) in category_manual_inputs.items():  # *** 确认是 category_id ***
                        category_id_str = str(category_id)
                        is_allowed = category_id_str in allowed_category_ids
                        print(
                            f"  Processing category_id: {category_id_str}, Allowed: {is_allowed}, Input: {input_data}"
                        )
                        ...
                        if isinstance(input_data, str):  # 如果前端只传单个字符串
                            ...
                            print(
                                f"  Skipping invalid input type for category {category_id_str}"
                            )
                            continue
                        ...
                        if is_allowed:
                            for manual_input in input_list:
                                if manual_input and manual_input.strip():
                                    print(
                                        f"    Adding manual input: '{manual_input.strip()}'"
                                    )
                                    values_str_manual.append("(%s, %s, %s)")
                                    params_manual.extend(
                                        [
                                            evaluation_id,
                                            category_id,
                                            manual_input.strip(),
                            ...
                            print(
                                f"    Skipping category {category_id_str} as manual input is not allowed."
                            )

                    if params_manual:
                        sql_manual = (
                            "INSERT INTO evaluation_manual_input (evaluation_id, category_id, manual_input) VALUES "
                            + ", ".join(values_str_manual)
                        )
                        # *** 打印最终的 SQL 和 参数 ***
        ...
        return jsonify(
            {"success": True, "message": "评价提交成功", "id": evaluation_id}
        ), 201
...
def delete_evaluation_route(evaluation_id):
    ...
    try:
        ...
        # 删除评价详情记录
        cur.execute(
            "DELETE FROM evaluation_detail WHERE evaluation_id = %s", (evaluation_id,)
        )
...
def ai_generate_route():
    ...
    current_user_id_for_log = (
        uuid.UUID(current_user_id_str) if current_user_id_str else None
    )
    ...
    try:
        ...
        task = run_llm_function_async.delay(
            ...
            context={
                ...
                "triggered_by_user_id": str(current_user_id_for_log)
                if current_user_id_for_log
                else None,
            },
        ...
        log.info(
            f"AI员工总结生成任务已提交 (Task ID: {task.id}) for user {evaluated_user_id}"
        )
        return jsonify(
            {
    ...
    try:
        ...
        for record in records:
            exam_record = {
                ...
                "exam_time": record["exam_time"].isoformat()
                if record["exam_time"]
                else None,
                ...
                "accuracy_rate": float(record["accuracy_rate"]) / 100
                if record["accuracy_rate"] is not None
                else 0.0,
                "course_names": [
                    course for course in record["course_names"] if course is not None
                ],
    ...
    try:
        ...
        if os.path.exists(user_avatar_path):
            ...
            return send_from_directory(
                avatar_folder, filename, as_attachment=False, mimetype="image/jpeg"
            )
        else:
            # 2. 如果文件不存在，快速返回 404
...
def get_tts_audio_storage_path():
    return current_app.config.get(
        ...
        os.path.join(current_app.root_path, "static", "tts_audio"),
    )


@app.route(
    "/media/tts_audio/<path:filepath>"
)  # 使用一个新的基础路径，例如 /media/tts_audio
...
def serve_tts_audio(filepath):
    ...
    if not full_path_to_file.startswith(os.path.abspath(storage_base_path)):
        current_app.logger.warning(
            f"路径遍历攻击检测: 请求路径 {full_path_to_file} 超出基础路径 {storage_base_path}"
        )
    ...
    try:
        ...
        current_app.logger.info(
            f"Serving audio file: directory='{directory}', filename='{filename}'"
        )
        return send_from_directory(
            directory, filename, as_attachment=False
        )  # as_attachment=False 表示浏览器直接播放
    except Exception as e:
