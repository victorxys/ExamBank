...
class Customer(Base):
    ...
    title: Mapped[Optional[str]] = mapped_column(
        String(50), comment="称谓 (先生/女士/小姐等)"
    )
    phone_number: Mapped[Optional[str]] = mapped_column(String(20), comment="联系电话")
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="创建时间"
    )
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="更新时间"
    )
...
class EmployeeSelfEvaluation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(
        Uuid, primary_key=True, server_default=text("gen_random_uuid()"), comment="主键"
    )
    ...
    evaluation_time: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="评价时间"
    )
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="更新时间"
    )
...
t_employee_self_evaluation_detail = Table(
    "employee_self_evaluation_detail",
    Base.metadata,
    Column(
        "id",
        Uuid,
...
class Exam(Base):
    ...
    exam_paper_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, comment="试卷ID，外键关联到 exampaper 表"
    )
    user_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, comment="用户ID，外键关联到 user 表"
    )
    ...
    correct_rate: Mapped[Optional[decimal.Decimal]] = mapped_column(
        Numeric(5, 2), comment="正确率"
    )
    knowledge_point_summary: Mapped[Optional[dict]] = mapped_column(
        JSONB, comment="知识点掌握情况 (JSON 数组)"
    )
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="创建时间"
    )
    ...
    finished: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("false"), comment="是否完成考试"
    )
...
class Exampaper(Base):
    ...
    course: Mapped[List["Trainingcourse"]] = relationship(
        "Trainingcourse", secondary="exampapercourse", back_populates="exam_paper"
    )
    exampaperquestion: Mapped[List["Exampaperquestion"]] = relationship(
        "Exampaperquestion", back_populates="exam_paper"
    )
...
class Trainingcourse(Base):
    ...
    course_name: Mapped[str] = mapped_column(
        String(255), comment="课程名称，不允许为空"
    )
    age_group: Mapped[Optional[str]] = mapped_column(
        String(50), comment='适用月龄 (例如："2-3个月", "3-4个月")'
    )
    ...
    exam_paper: Mapped[List["Exampaper"]] = relationship(
        "Exampaper", secondary="exampapercourse", back_populates="course"
    )
    knowledgepoint: Mapped[List["Knowledgepoint"]] = relationship(
...
class User(Base):
    ...
    role: Mapped[str] = mapped_column(
        String(50),
        server_default=text("'student'::character varying"),
    ...
    myms_user_id: Mapped[Optional[int]] = mapped_column(
        BigInteger, comment="关联到另一个数据库中用户的 ID"
    )
...
t_answerrecord = Table(
    ...
    ForeignKeyConstraint(
        ["exam_id"], ["exam.id"], ondelete="CASCADE", name="answerrecord_exam_id_fkey"
    ),
...
t_exampapercourse = Table(
    ...
    ForeignKeyConstraint(
        ...
        ["trainingcourse.id"],
        ondelete="CASCADE",
        name="exampapercourse_course_id_fkey",
    ),
    ForeignKeyConstraint(
        ["exam_paper_id"],
        ["exampaper.id"],
        ondelete="CASCADE",
        name="exampapercourse_exam_paper_id_fkey",
    ),
...
class Knowledgepoint(Base):
    ...
    course_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, comment="外键，关联到 TrainingCourse 表"
    )
    ...
    course: Mapped["Trainingcourse"] = relationship(
        "Trainingcourse", back_populates="knowledgepoint"
    )
    question: Mapped[List["Question"]] = relationship(
        "Question", back_populates="knowledge_point"
    )
...
class UserProfile(User):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, primary_key=True, comment="用户ID，主键，外键关联到 user 表"
    )
    profile_data: Mapped[dict] = mapped_column(
        JSONB, comment="用户详细信息 (JSON 格式)"
    )
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="创建时间"
    )
...
class Question(Base):
    ...
    knowledge_point_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, comment="外键，关联到 KnowledgePoint 表"
    )
    question_type: Mapped[str] = mapped_column(
        String(50), comment='题目类型 (例如："单选", "多选", "问答")'
    )
    ...
    knowledge_point: Mapped["Knowledgepoint"] = relationship(
        "Knowledgepoint", back_populates="question"
    )
    exampaperquestion: Mapped[List["Exampaperquestion"]] = relationship(
        "Exampaperquestion", back_populates="question"
    )
...
class Exampaperquestion(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["exampaper.id"],
            ondelete="CASCADE",
            name="exampaperquestion_exam_paper_id_fkey",
        ),
        ForeignKeyConstraint(
            ["question_id"],
            ["question.id"],
            ondelete="CASCADE",
            name="exampaperquestion_question_id_fkey",
        ),
    ...
    exam_paper_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, comment="外键，关联到 ExamPaper 表"
    )
    question_id: Mapped[uuid.UUID] = mapped_column(
        Uuid, comment="外键，关联到 Question 表"
    )
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="创建时间"
    )
    ...
    exam_paper: Mapped["Exampaper"] = relationship(
        "Exampaper", back_populates="exampaperquestion"
    )
    question: Mapped["Question"] = relationship(
        "Question", back_populates="exampaperquestion"
    )
...
class Option(Base):
    ...
    is_correct: Mapped[Optional[bool]] = mapped_column(
        Boolean, server_default=text("false"), comment="是否为正确答案"
    )
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(
        TIMESTAMP(True, 6), server_default=text("now()"), comment="创建时间"
    )
