...
# --- 模块级的全局变量 (保持不变) ---
USAGE_FILE_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'instance', 'usage_data.json'))
FILE_LOCK = Lock()
print("✅ 已加载代理池，包含以下服务器:")
for server in PROXY_POOL:
    print(f" - {server['id']} (API Key: {server['api_key']}, Proxy URL: {server['proxy_url']})") 
...
def get_next_identity():
    ...
    with FILE_LOCK:
        ...
        print(f"--- [代理轮询] 选择使用: {server_id} (今日已用: {usage_data[server_id]} 次) ---")
