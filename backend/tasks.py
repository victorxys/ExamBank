...
# 1.1: 注册可被异步调用的LLM函数
LLM_FUNCTION_REGISTRY = {"transform_text_with_llm": transform_text_with_llm}
...
def _handle_oral_script_result(result, context):
    ...
    latest_oral_script = (
        TtsScript.query.filter_by(
            training_content_id=training_content_id, script_type="oral_script"
        )
    ...
    logger.info(
        f"口播稿已生成并保存，内容 {training_content_id} 状态更新为 pending_tts_refine。"
    )
...
def _handle_final_refine_result(result, context):
    ...
    latest_final_script = (
        TtsScript.query.filter_by(
            training_content_id=training_content_id, script_type="final_tts_script"
        )
        .order_by(TtsScript.version.desc())
        .first()
    )
    ...
    logger.info(
        f"最终TTS脚本已生成 (ID: {new_script.id})，内容 {training_content_id} 状态更新为 pending_sentence_split。"
    )
...
@celery_app.task(
    bind=True,
    name="tasks.run_llm_function_async",
    max_retries=2,
    default_retry_delay=60,
)
def run_llm_function_async(
    self, llm_function_identifier, callback_identifier, context, *args, **kwargs
):
    ...
    with app.app_context():
        ...
        try:
            logger.info(
                f"任务 {self.request.id}: 开始执行 LLM 函数 '{llm_function_identifier}'..."
            )
            self.update_state(
                state="PROGRESS",
                meta={
                    "message": f"正在调用LLM ({llm_function_identifier})...",
                    "context": context,
                },
            )
            ...
            logger.info(
                f"任务 {self.request.id}: LLM 函数 '{llm_function_identifier}' 执行成功。"
            )
            self.update_state(
                state="PROGRESS",
                meta={"message": "LLM调用成功，正在处理结果...", "context": context},
            )
            ...
            db.session.commit()
            logger.info(
                f"任务 {self.request.id}: 回调 '{callback_identifier}' 执行完毕，数据已提交。"
            )
            return {
                "status": "Success",
                "message": "任务成功完成",
                "result_preview": llm_result,
            }
            # return {'status': 'Success', 'message': '任务成功完成', 'result_preview': str(llm_result)[:100] + '...'}
        except Exception as e:
            db.session.rollback()
            logger.error(
                f"任务 {self.request.id} 在执行 '{llm_function_identifier}' 或回调 '{callback_identifier}' 时失败: {e}",
                exc_info=True,
            )
            try:
                # 触发Celery的重试机制
...
def _save_audio_file(
    audio_binary_content,
    training_content_id_str,
    sentence_id_str,
    version,
    extension=".wav",
):
    app = create_flask_app_for_task()
    with app.app_context():
        storage_base_path = app.config.get(
            "TTS_AUDIO_STORAGE_PATH", os.path.join(app.root_path, "static", "tts_audio")
        )
        ...
        file_name = (
            f"sentence_v{version}_{timestamp_str}{extension}"  # <--- 使用 extension
        )
...
def _create_new_script_version_task(
    source_script_id, training_content_id, new_script_type, new_content, llm_log_id=None
):
    ...
    app = create_flask_app_for_task()
    with app.app_context():
        latest_same_type_script = (
            TtsScript.query.filter_by(
                training_content_id=training_content_id, script_type=new_script_type
            )
            .order_by(TtsScript.version.desc())
            .first()
        )
...
def trigger_tts_refine_async(self, oral_script_id_str):
    ...
    with app.app_context():
        ...
        try:
            tts_service_base_url = app.config.get(
                "TTS_SERVICE_BASE_URL", "http://test.mengyimengsao.com:37860/"
            )
            gradio_tts_client = GradioClient(tts_service_base_url)

            job = gradio_tts_client.predict(
                text_file=oral_script.content,
                oral=2,
                laugh=0,
                bk=4,
                temperature=0.1,
                top_P=0.7,
                top_K=20,
                api_name="/generate_refine",
            )

            refined_content = None
            if hasattr(job, "result"):
                refined_content = job.result()
            elif isinstance(job, tuple) and len(job) > 0:
                refined_content = job[0]
            elif isinstance(job, str):
                refined_content = job
            else:
                raise Exception("Gradio TTS API 返回格式未知")
            ...
            logger.error(
                f"TTS Refine Task 失败 for script {oral_script_id_str}: {e}",
                exc_info=True,
            )
...
def generate_single_sentence_audio_async(
    self,
    sentence_id_str,
    tts_engine_identifier="gradio_default",
    pt_file_path_relative=None,
    tts_engine_params=None,
    ...
    with app.app_context():
        ...
        if not sentence:
            ...
            self.update_state(
                state="FAILURE",
                meta={"exc_type": "ValueError", "exc_message": "句子未找到"},
            )
        ...
        if not training_content:
            logger.error(
                f"GenerateAudio Task: 未找到句子 {sentence_id_str} 关联的 training_content。"
            )
            self.update_state(
                state="FAILURE",
                meta={"exc_type": "ValueError", "exc_message": "未找到关联的培训内容"},
            )
        ...
        # 1. 从App配置加载系统默认值
        final_config.update(
            current_app.config.get("DEFAULT_TTS_CONFIG", {})
        )  # 假设你在app.py中定义了DEFAULT_TTS_CONFIG
        ...
        logger.info(
            f"[AudioTask:{self.request.id}] Sentence {sentence_id_str} | Final TTS Config: {final_config}"
        )
        ...
        try:
            ...
            if tts_engine_identifier == "gemini_tts":
                logger.info(
                    f"GenerateAudio Task: Using Gemini TTS for sentence {sentence_id_str}"
                )
                ...
                text_to_speak = (
                    final_config.get("system_prompt", "") + sentence.sentence_text
                )
                ...
                logger.info(
                    f"Successfully retrieved identity '{api_key_name}' from manager."
                )
                # ++++++++++++++++ Celery 内部对照诊断 ++++++++++++++++
                ...
                try:
                    ...
                    with httpx.Client(mounts=mounts, timeout=15.0) as client:
                        logger.info(
                            "[诊断] 独立的 httpx.Client 已创建，正在发送请求..."
                        )
                        response = client.get(test_url)
                        logger.info(
                            f"[诊断] 独立的 httpx 请求已收到响应，状态码: {response.status_code}"
                        )
                        if response.status_code == 200:
                            logger.info(
                                f"✅ [诊断成功] 独立的 httpx 调用{api_key_name}成功！响应: {response.json()}"
                            )
                        else:
                            logger.error(
                                f"❌ [诊断失败] 独立的 httpx 调用失败！状态码: {response.status_code}"
                            )
                except Exception as e:
                    logger.error(
                        f"❌ [诊断失败] 独立的 httpx 调用{api_key_name}时发生致命错误: {type(e).__name__}: {e}",
                        exc_info=True,
                    )
                ...
                # --- 调用您真正的业务逻辑 ---
                logger.info(
                    "--- [业务逻辑开始] 正在调用 generate_audio_with_gemini_tts ---"
                )

                audio_binary_content, output_audio_mime_type = (
                    generate_audio_with_gemini_tts(
                        text_to_speak=text_to_speak,
                        api_key=identity["api_key"],
                        model_name=final_config.get(
                ...
                logger.info(
                    "--- [业务逻辑结束] 调用 generate_audio_with_gemini_tts 成功 ---"
                )
                ...
                if pt_file_path_relative:
                    ...
                    if os.path.exists(absolute_pt_file_path):
                        ...
                        logger.warning(
                            f"[SingleAudioTask] 指定的音色文件不存在: {absolute_pt_file_path}，将使用默认音色。"
                        )
                else:
                ...
                if isinstance(job_result, tuple) and len(job_result) > 0:
                    ...
                    if (
                        isinstance(output_component, dict)
                        and output_component.get("is_file")
                        and output_component.get("name")
                    ):
                        ...
                        output_audio_mime_type = (
                            mimetypes.guess_type(audio_file_path_from_gradio)[0]
                            or "audio/wav"
                        )
                    elif isinstance(
                        output_component, str
                    ) and output_component.startswith("data:audio/"):
                ...
                elif isinstance(job_result, str) and os.path.exists(
                    job_result
                ):  # 如果直接返回有效路径
                ...
                if (
                    "pt_file" in actual_generation_params_for_log
                    and actual_generation_params_for_log["pt_file"] is not None
                ):
                    # 只记录路径，而不是 FileData 对象
                    actual_generation_params_for_log["pt_file_path"] = (
            ...
            TtsAudio.query.filter_by(
                tts_sentence_id=sentence.id, is_latest_for_sentence=True
            ).update({"is_latest_for_sentence": False})
            new_version = 1
            latest_audio_for_sentence = (
                TtsAudio.query.filter_by(tts_sentence_id=sentence.id)
                .order_by(TtsAudio.version.desc())
                .first()
            )
            ...
            if output_audio_mime_type == "audio/wav":
                file_extension = ".wav"

            relative_path, file_size = _save_audio_file(
                audio_binary_content,
                training_content.id,
                sentence.id,
                new_version,
            ...
            new_audio_record = TtsAudio(
                ...
                voice_name=actual_generation_params_for_log.get("voice_name")
                or actual_generation_params_for_log.get("roleid")
                or "default",
            ...
            db.session.commit()
            logger.info(
                f"GenerateAudio Task (Engine: {tts_engine_identifier}): 句子 {sentence_id_str} 的音频 (ID: {new_audio_record.id}) 已成功生成并保存。"
            )
            return {
                "status": "Success",
                "audio_id": str(new_audio_record.id),
                "file_path": relative_path,
            ...
            logger.error(
                f"GenerateAudio Task: Exception for sentence {sentence_id_str} (Engine: {tts_engine_identifier}): {e}",
                exc_info=True,
            )
            if sentence:
            ...
            return {
                "status": "Error",
                "message": f"生成单句语音 ({tts_engine_identifier}) 时发生服务器错误: "
                + str(e),
            }


@celery_app.task(
    bind=True, name="tasks.batch_generate_audio_task", max_retries=1
)  # 保持原来的 name 或更新
def batch_generate_audio_task(
    self,
    final_script_id_str,
    tts_engine_identifier="gradio_default",  # 新增：TTS 引擎标识符
    pt_file_path_relative_for_gradio=None,  # 新增：用于 Gradio 的 PT 文件路径
    tts_engine_params_for_all=None,
):  # 新增：传递给每个子任务的引擎特定参数
    app = create_flask_app_for_task()
    with app.app_context():
        try:  # 顶层 try-except 块，用于捕获批量任务本身的初始化错误
            ...
            logger.info(
                f"[BatchTask:{self.request.id}] 开始处理批量语音生成，脚本ID: {final_script_id}, 引擎: {tts_engine_identifier}"
            )
            ...
            if initially_to_process_count == 0:
                logger.info(
                    f"[BatchTask:{self.request.id}] 脚本 {final_script_id} 没有需要生成语音的句子。"
                )
                all_sentences_in_script = script.tts_sentences.all()
                all_generated = all(
                    s.audio_status == "generated" for s in all_sentences_in_script
                )
            ...
            logger.info(
                f"[BatchTask:{self.request.id}] 将为 {initially_to_process_count} 个句子派发语音生成子任务 (引擎: {tts_engine_identifier})"
            )
            ...
            for index, sentence in enumerate(sentences_to_process):
                ...
                try:
                    ...
                    sub_task = generate_single_sentence_audio_async.delay(
                        ...
                        pt_file_path_relative=pt_file_path_relative_for_gradio,
                        tts_engine_params=(tts_engine_params_for_all or {}),
                    )
                    submitted_subtasks_info.append(
                        {  # <--- 收集子任务信息
                    ...
                    logger.info(
                        f"[BatchTask:{self.request.id}] ({index + 1}/{initially_to_process_count}) 为句子 {sentence.id} 提交了子任务 {sub_task.id} (引擎: {tts_engine_identifier})"
                    )
                    ...
                    logger.error(
                        f"[BatchTask:{self.request.id}] 提交句子 {sentence.id} 的子任务失败: {e_dispatch}",
                        exc_info=True,
                    )
            ...
            if failed_to_submit_count > 0 and len(submitted_subtasks_info) == 0:
                ...
                training_content.status = (
                    "audio_generation_in_progress"  # 或保持 processing_batch_audio
                )
            ...
            logger.error(
                f"[BatchTask:{self.request.id}] 批量任务初始化失败: {e_batch_init}",
                exc_info=True,
            )
            ...
            return {
                "status": "Error",
                "message": f"批量语音生成任务失败: {str(e_batch_init)}",
            }
...
def merge_all_audios_for_content(self, training_content_id_str):
    ...
    with app.app_context():
        ...
        logger.info(
            f"[MergeTask:{task_id}] 开始音频合并任务 for TrainingContent ID: {training_content_id_str}"
        )

        content = TrainingContent.query.get(training_content_id_str)
        if not content:
            logger.error(
                f"[MergeTask:{task_id}] TrainingContent {training_content_id_str} not found."
            )
        ...
        if not latest_final_script:
            logger.warning(
                f"[MergeTask:{task_id}] No final_tts_script found for content {content.id}."
            )
        ...
        sentences_in_order = latest_final_script.tts_sentences.order_by(
            TtsSentence.order_index
        ).all()
        if not sentences_in_order:
            logger.warning(
                f"[MergeTask:{task_id}] No sentences in final_tts_script {latest_final_script.id}."
            )
            content.status = "merge_failed_no_sentences"
            db.session.commit()
            self.update_state(
                state="SUCCESS",
                meta={"message": "No sentences to merge", "total_segments": 0},
            )  # Success, as nothing to do
            return {
                "status": "Success",
                "message": "No sentences to merge",
                "total_segments": 0,
            }
        ...
        for i, sentence_obj in enumerate(sentences_in_order):
            ...
            if (
                not latest_audio_for_sentence
                or sentence_obj.audio_status != "generated"
            ):
                all_audios_generated = False
                logger.error(
                    f"[MergeTask:{task_id}] Sentence {sentence_obj.id} (Order: {sentence_obj.order_index}) audio not generated or not found. Status: {sentence_obj.audio_status}"
                )
                ...
                self.update_state(
                    state="FAILURE",
                    meta={
                        "error": f"Audio for sentence order {sentence_obj.order_index + 1} is not ready."
                    },
                )
                return {
                    "status": "Error",
                    "message": f'Audio for sentence (Order: {sentence_obj.order_index + 1}) "{sentence_obj.sentence_text[:30]}..." is not ready.',
                }

            # Construct full path to the audio file
            storage_base_path = app.config.get(
                "TTS_AUDIO_STORAGE_PATH",
                os.path.join(app.root_path, "static", "tts_audio"),
            )
            audio_full_path = os.path.join(
                storage_base_path, latest_audio_for_sentence.file_path
            )

            if not os.path.exists(audio_full_path):
                all_audios_generated = False
                logger.error(
                    f"[MergeTask:{task_id}] Audio file {audio_full_path} for sentence {sentence_obj.id} not found on disk."
                )
                ...
                self.update_state(
                    state="FAILURE",
                    meta={
                        "error": f"Audio file for sentence order {sentence_obj.order_index + 1} missing on disk."
                    },
                )
                return {
                    "status": "Error",
                    "message": f"Audio file for sentence (Order: {sentence_obj.order_index + 1}) missing on disk.",
                }
        ...
        if (
            not all_audios_generated
        ):  # Should have been caught above, but as a safeguard
            return  # Already handled

        logger.info(
            f"[MergeTask:{task_id}] All {len(sentence_audio_paths)} sentence audios are ready for merging."
        )
        ...
        try:
            for i, audio_info in enumerate(sentence_audio_objects):
                try:
                    ...
                    logger.error(
                        f"[MergeTask:{task_id}] Could not decode audio file: {audio_info['file_path']}. Skipping or failing."
                    )
                    content.status = (
                        f'merge_failed_decode_error_sent_{audio_info["order_index"]+1}'
                    )
                    db.session.commit()
                    self.update_state(
                        state="FAILURE",
                        meta={
                            "error": f'Could not decode audio for sentence order {audio_info["order_index"] + 1}'
                        },
                    )
                    return {
                        "status": "Error",
                        "message": f'Error decoding audio for sentence {audio_info["order_index"] + 1}',
                    }
            ...
            if merged_sound is None:
                logger.warning(
                    f"[MergeTask:{task_id}] No audio segments were actually merged (e.g., empty list)."
                )
                content.status = "merge_failed_no_segments_processed"
                db.session.commit()
                self.update_state(
                    state="SUCCESS", meta={"message": "No audio segments to merge."}
                )
            ...
            latest_merged_version_obj = (
                TtsAudio.query.with_entities(func.max(TtsAudio.version))
                .filter_by(training_content_id=content.id, audio_type="merged_audio")
                .scalar()
            )
            ...
            merged_relative_path, merged_file_size = _save_merged_audio_file(
                merged_sound, content.id, new_merged_version
            )
            logger.info(
                f"[MergeTask:{task_id}] 合并音频已保存，相对路径: {merged_relative_path}"
            )
            ...
            for i, segment_data in enumerate(new_segments_data):
                ...
                srt_content_lines.append(
                    segment_data["original_sentence_text_ref"] or ""
                )
                srt_content_lines.append("")  # 每个字幕块后的空行

            srt_content = "\n".join(srt_content_lines)
            logger.info(
                f"[MergeTask:{task_id}] SRT内容已生成，共 {len(new_segments_data)} 条字幕。"
            )
            ...
            if not os.path.exists(srt_txt_dir):
                logger.info(
                    f"[MergeTask:{task_id}] 目标目录不存在，正在创建: {srt_txt_dir}"
                )
            ...
            try:
                with open(srt_txt_full_path, "w", encoding="utf-8") as f:
                    f.write(srt_content)
                logger.info("成功！TXT字幕文件已写入磁盘。")

                # 验证文件是否真的创建成功
                if not os.path.exists(srt_txt_full_path):
                    logger.error(
                        f"[MergeTask:{task_id}] 严重错误：文件写入后，os.path.exists() 检查失败！路径: {srt_txt_full_path}"
                    )
                else:
                    logger.info("os.path.exists() 确认文件已在磁盘上。")

            except (IOError, OSError) as e_write:
                logger.error(
                    f"[MergeTask:{task_id}] 写入TXT文件时发生IO错误: {e_write}",
                    exc_info=True,
                )
            ...
            for seg_data in new_segments_data:
                segment_entry = MergedAudioSegment(
                    merged_audio_id=new_merged_audio_record.id, **seg_data
                )
            ...
            if sentence_ids_in_script:
                ...
                logger.info(
                    f"[MergeTask:{task_id}] 已重置 {len(sentence_ids_in_script)} 个句子的 'modified_after_merge' 标记。"
                )
            # ------------------------------------------------->>>
            ...
            logger.info(
                f"[MergeTask:{task_id}] Audio merge successful for content {content.id}. New merged audio ID: {new_merged_audio_record.id}"
            )
            self.update_state(
                state="SUCCESS",
                meta={
            ...
            db.session.rollback()
            logger.error(
                f"[MergeTask:{task_id}] Error during audio merging process for {content.id}: {e}",
                exc_info=True,
            )
            content.status = "merge_failed_exception"
            db.session.commit()
            self.update_state(
                state="FAILURE", meta={"error": str(e), "exc_type": type(e).__name__}
            )
...
def analyze_video_script_task(self, synthesis_id_str):
    ...
    with app.app_context():
        ...
        try:
            ...
            base_web_path = os.path.join(
                "video_synthesis",
                str(synthesis_task.training_content_id),
            ...
            logger.info(
                f"[AnalyzeTask:{self.request.id}] PDF转换完成，共 {len(image_relative_paths)} 张图片。"
            )
            ...
            db.session.commit()
            logger.error(
                f"视频脚本分析任务失败 (ID: {synthesis_id_str}): {e}", exc_info=True
            )
            self.update_state(
                state="FAILURE",
                meta={"exc_type": type(e).__name__, "exc_message": str(e)},
...
class CeleryProgressLogger:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if "message" in kwargs:
            ...
            if "writing audio" in line.lower():
                mapped_progress = 46  # 音频写入阶段
            elif "building video" in line.lower():
                mapped_progress = 48  # 视频构建阶段
        ...
        if match and self.total_duration > 0:
            try:
                ...
                processed_seconds = (
                    hours * 3600 + minutes * 60 + seconds + hundredths / 100.0
                )
...
def synthesize_video_task(self, synthesis_id_str):
    ...
    with app.app_context():
        task_id = self.request.id
        logger.info(
            f"[VideoSynthTask:{task_id}] ------------------------------------------"
        )
        logger.info(
            f"[VideoSynthTask:{task_id}] 开始视频合成任务 for Synthesis ID: {synthesis_id_str}"
        )
        ...
        try:
            ...
            if not os.path.exists(pdf_path) or not os.path.exists(audio_path):
                raise FileNotFoundError(
                    f"PDF或音频素材文件pdf_path: {pdf_path} 或audio_path: {audio_path}不存在。"
                )
            ...
            images = convert_from_path(
                pdf_path, output_folder=pdf_images_dir, fmt="png", output_file="slide_"
            )
            ...
            image_paths = sorted(
                [img.filename for img in images], key=get_page_number_from_filename
            )

            # image_paths = sorted([img.filename for img in images], key=lambda x: int(os.path.basename(x).split('_')[1].split('.')[0]))
            logger.info(
                f"[VideoSynthTask:{task_id}] 已成功转换 {len(image_paths)} 页PDF。"
            )
            self.update_state(
                state="PROGRESS",
                meta={
                    "current_step": "pdf_conversion",
                    "message": f"已成功转换 {len(image_paths)} 页PDF。",
                },
            )  # Removed extra backslash
            ...
            self.update_state(
                state="PROGRESS",
                meta={
                    "current_step": "clips_creation",
                    "progress": 20,
                    "message": "步骤 3: 正在创建视频剪辑...",
                },
            )  # Removed extra backslash
            ...
            if (width != initial_width) or (height != initial_height):
                ...
                logger.info(
                    f"[VideoSynthTask:{self.request.id}] 视频标准尺寸确定为: {video_size}"
                )

            self.update_state(
                state="PROGRESS",
                meta={
                    "current_step": "audio_loading",
                    "message": "步骤 3: 正在加载音频...",
                },
            )  # Removed extra backslash
            ...
            for script_item in video_scripts:
                ...
                start_seconds = sum(
                    x * float(t)
                    for x, t in zip(
                        [3600, 60, 1], start_str.replace(",", ".").split(":")
                    )
                )
                end_seconds = sum(
                    x * float(t)
                    for x, t in zip([3600, 60, 1], end_str.replace(",", ".").split(":"))
                )
                ...
                if 0 <= image_path_index < len(image_paths):
                    ...
                    logger.debug(
                        f"[VideoSynthTask:{task_id}]   - 处理剪辑: 页码 {ppt_page_num}, 路径 {img_path}, 时长 {duration}s"
                    )
            ...
            logger.info(
                f"[VideoSynthTask:{task_id}] 正在拼接 {len(clips)} 个视频剪辑..."
            )
            self.update_state(
                state="PROGRESS",
                meta={
                    "current_step": "concatenation",
                    "progress": 40,
                    "message": f"正在拼接 {len(clips)} 个视频剪辑...",
                },
            )  # Removed extra backslash

            final_video = concatenate_videoclips(clips, method="compose").set_audio(
                audio_clip
            )
            ...
            final_video_to_write = (
                final_video.subclip(0, DEBUG_DURATION_SECONDS)
                if IS_DEBUG_MODE
                else final_video
            )
            # --------------
            ...
            self.update_state(
                state="PROGRESS",
                meta={"current_step": "set_audio", "message": "正在设置音频..."},
            )  # Removed extra backslash
            ...
            video_output_dir = os.path.join(
                ...
                "uploads",
                "course_resources",
                str(synthesis_task.training_content.course_id),
            )
            ...
            self.update_state(
                state="PROGRESS",
                meta={
                    "current_step": "saving_to_db",
                    "progress": 98,
                    "message": "步骤 6: 正在保存记录到数据库...",
                },
            )  # Removed extra backslash
            course_id_str = str(synthesis_task.training_content.course_id)
            relative_file_path = os.path.join(
                "uploads", "course_resources", course_id_str, video_filename
            )
            ...
            logger.info(
                f"[VideoSynthTask:{task_id}] 视频合成成功！新的资源ID: {new_resource.id}"
            )
            ...
            db.session.commit()
            logger.error(
                f"视频合成任务失败 (ID: {synthesis_id_str}): {e}", exc_info=True
            )
            self.update_state(
                state="FAILURE",
                meta={"exc_type": type(e).__name__, "exc_message": str(e)},
...
@celery_app.task(
    bind=True, name="tasks.merge_current_generated_audios_async", max_retries=1
)
def merge_current_generated_audios_async(self, training_content_id_str):
    ...
    with app.app_context():
        ...
        if not content:
            logger.error(
                f"[MergeCurrentTask:{self.request.id}] TrainingContent {training_content_id_str} 未找到。"
            )
            self.update_state(state="FAILURE", meta={"error": "培训内容未找到"})
            return {"status": "Error", "message": "培训内容未找到"}

        latest_final_script = (
            db.session.query(TtsScript)
            .filter(
        ...
        if not latest_final_script:
            logger.error(
                f"[MergeCurrentTask:{self.request.id}] 未找到内容 {content.id} 的最终脚本。"
            )
            self.update_state(state="FAILURE", meta={"error": "最终脚本未找到"})
            return {"status": "Error", "message": "最终脚本未找到"}

        # 找到该最终脚本下所有已成功生成语音的句子，并按顺序排列
        generated_sentences_with_audio = (
            db.session.query(TtsSentence, TtsAudio)
            .join(TtsAudio, TtsSentence.id == TtsAudio.tts_sentence_id)
            .filter(
                TtsSentence.tts_script_id == latest_final_script.id,
                TtsSentence.audio_status == "generated",
                TtsAudio.is_latest_for_sentence,
            )
            .order_by(TtsSentence.order_index)
            .all()
        )
        ...
        if not generated_sentences_with_audio:
            logger.info(
                f"[MergeCurrentTask:{self.request.id}] 内容 {content.id} (脚本 {latest_final_script.id}) 没有已生成的语音可以合并。"
            )
        ...
        logger.info(
            f"[MergeCurrentTask:{self.request.id}] 找到 {len(generated_sentences_with_audio)} 个已生成的语音准备合并 for content {content.id}"
        )
        content.status = "merging_audio"
        db.session.commit()
        ...
        try:
            for sentence, audio_record in generated_sentences_with_audio:
                audio_file_full_path = os.path.join(
                    audio_storage_base, audio_record.file_path
                )
                if not os.path.exists(audio_file_full_path):
                    logger.warning(
                        f"[MergeCurrentTask:{self.request.id}] 音频文件不存在: {audio_file_full_path} for sentence {sentence.id}。跳过此句。"
                    )
                    continue
                ...
                try:
                    # 根据文件扩展名加载音频
                    file_ext = (
                        os.path.splitext(audio_record.file_path)[1].lower().lstrip(".")
                    )
                    if not file_ext:  # 如果没有扩展名，尝试根据MIME类型或默认
                        # 这里可以添加更复杂的逻辑，例如从 audio_record.generation_params 或 mime_type 推断
                        file_ext = "wav"  # 或 "mp3"
                        logger.warning(
                            f"音频文件 {audio_record.file_path} 无扩展名, 尝试作为 {file_ext} 加载。"
                        )

                    sound_segment = AudioSegment.from_file(
                        audio_file_full_path, format=file_ext
                    )
                    ...
                    logger.error(
                        f"[MergeCurrentTask:{self.request.id}] 合并句子 {sentence.id} 的音频失败: {e_segment}",
                        exc_info=True,
                    )
                    # 可以选择是跳过这个句子还是中止整个合并
                    # 这里选择跳过
            ...
            logger.info(
                f"[MergeCurrentTask:{self.request.id}] 内容 {content.id} 的音频已成功合并到 {merged_relative_path}"
            )
            final_meta = {
                "total_sentences": len(generated_sentences_with_audio),
                "merged_count": processed_count,
            ...
            logger.error(
                f"[MergeCurrentTask:{self.request.id}] 合并音频过程中发生错误 for content {content.id}: {e}",
                exc_info=True,
            )
...
def _find_reusable_audio_info(text_to_match, training_content_id_to_match):
    ...
    reusable_sentence_with_audio = (
        db.session.query(TtsSentence, TtsAudio)
        .join(TtsAudio, TtsSentence.id == TtsAudio.tts_sentence_id)
        .join(
            TtsScript,
            TtsSentence.tts_script_id == TtsScript.id,  # <--- 加入 TtsScript
        )
        .filter(
            TtsScript.training_content_id
            == training_content_id_to_match,  # <--- 通过 TtsScript 过滤
...
def resplit_and_match_sentences_task(self, final_tts_script_id_str):
    ...
    with app.app_context():
        ...
        if not final_script or final_script.script_type != "final_tts_script":
            logger.error(
                f"[ResplitTask:{self.request.id}] 脚本 {final_tts_script_id_str} 未找到或非最终脚本。"
            )
        ...
        if not training_content:
            logger.error(
                f"[ResplitTask:{self.request.id}] 脚本 {final_script.id} 未关联培训内容。"
            )
            raise ValueError("脚本未关联培训内容")

        logger.info(
            f"[ResplitTask:{self.request.id}] 开始为脚本 {final_script.id} (v{final_script.version}) 重新拆分和匹配句子。"
        )
        ...
        try:
            ...
            new_sentence_texts = [
                s.strip() for s in final_script.content.split("\n") if s.strip()
            ]
            ...
            for index, text in enumerate(new_sentence_texts):
                ...
                reusable_audio_info = _find_reusable_audio_info(
                    text, training_content.id
                )
                ...
                if reusable_audio_info:
                    ...
                    new_audio_record = TtsAudio(
                        ...
                        tts_engine=reusable_audio_info[
                            "tts_engine"
                        ],  # 可以记录为 "reused_from_" + original_engine
                    ...
                    logger.info(
                        f"[ResplitTask:{self.request.id}] 句子 '{text[:30]}...' (新ID: {new_sentence.id}) 复用了音频: {reusable_audio_info['file_path']}"
                    )
            ...
            training_content.status = (
                "pending_audio_generation"  # 或 'audio_matching_complete'
            )
            db.session.commit()
            logger.info(
                f"[ResplitTask:{self.request.id}] 脚本 {final_script.id} 重新拆分完成。创建/更新句子数: {created_count}，复用音频数: {reused_audio_count}"
            )
            return {
                "status": "Success",
                "message": f"句子已重新处理，复用音频 {reused_audio_count} 个。",
                "created_sentences": created_count,
                "reused_audios": reused_audio_count,
            }

        except Exception as e:
            db.session.rollback()
            logger.error(
                f"[ResplitTask:{self.request.id}] 重新拆分和匹配句子失败 for script {final_script.id}: {e}",
                exc_info=True,
            )
            if training_content:
...
def auto_check_and_extend_renewal_bills_task(self):
    ...
    with app.app_context():
        ...
        task_logger.info(
            "[AutoRenewCheckTask] 开始执行每周自动续约账单检查和扩展任务..."
        )
...
def sync_all_contracts_task(self):
    ...
    with app.app_context():
        logger.info(
            f"[ContractSyncTask:{self.request.id}] Starting contract sync task..."
        )
        FORM_CONFIGS = [
            {
                "form_token": "Iqltzj",
                ...
                "mapping": {
                    ...
                    "customer_phone":{
                        "field_id": "field_1",
                        "is_association": True,
                        "associated_field_id": "field_3",
                    },
                    "customer_id_card":{
                        "field_id": "field_1",
                        "is_association": True,
                        "associated_field_id": "field_4",
                    },
                    "customer_address":{
                        "field_id": "field_1",
                        "is_association": True,
                        "associated_field_id": "field_5",
                    },
            ...
            },
            {
                ...
                "mapping": {
                    ...
                    "introduction_fee":{
                        "field_id": "field_1",
                        "is_association": True,
                        "associated_field_id": "field_13",
                    },
                    "notes":{
                        "field_id": "field_1",
                        "is_association": True,
                        "associated_field_id": "field_18",
                    },
                    "employee_name": {"field_id": "field_2"},
                    "employee_phone": {"field_id": "field_3"},
                    "employee_id_card": {"field_id": "field_4"},
        ...
        try:
            ...
            for config in FORM_CONFIGS:
                new_count, skipped_count, newly_synced_ids = (
                    sync_service.sync_contracts_from_form(
            ...
            if all_new_contract_ids:
                logger.info(
                    f"[ContractSyncTask:{self.request.id}] Triggering bill pre-calculation for {len(all_new_contract_ids)} new contracts."
                )
            ...
            logger.error(
                f"[ContractSyncTask:{self.request.id}] Sync failed: {e}", exc_info=True
            )
            self.update_state(state="FAILURE", meta={"error": str(e)})
            raise
...
def calculate_monthly_billing_task(
    year, month, contract_id=None, force_recalculate=False
):
    app = create_flask_app_for_task()
    with app.app_context():
        task_logger = get_task_logger(__name__)
...
def generate_all_bills_task(contract_id):
    ...
    with app.app_context():
        ...
        try:
            task_logger.info(
                f"Starting full lifecycle bill generation for Contract: {contract_id}"
            )
            engine = BillingEngine()
            engine.generate_all_bills_for_contract(contract_id)
            task_logger.info(
                f"Finished full lifecycle bill generation for Contract: {contract_id}"
            )
            return {
                "status": "Success",
                "message": f"All bills for contract {contract_id} have been generated.",
            }
        except Exception as e:
            task_logger.error(
                f"Full lifecycle bill generation for Contract {contract_id} failed: {e}",
                exc_info=True,
            )
            raise
...
@celery_app.task(name="tasks.post_virtual_contract_creation", bind=True, max_retries=5, default_retry_delay=10)
...
def send_wechat_notification_task(touser, title, description, jump_url, msg_type):
    ...
    with app.app_context():
        ...
        try:
            import json
            # 获取实际被投递的 touser
            from backend.utils.wechat_notifier import get_wechat_config, normalize_wechat_touser
            ...
            if not target_user:
                target_user = "@all"
            has_partial_failure = any(raw_result.get(key) for key in ("invaliduser", "invalidparty", "invalidtag"))
            log_status = "PARTIAL_FAILED" if success and has_partial_failure else ("SUCCESS" if success else "FAILED")
            ...
            log_record = WechatMessageLog(
                ...
                jump_url=jump_url,
                status=log_status,
                error_details=json.dumps(raw_result, ensure_ascii=False) if (not success or has_partial_failure) else None
            )
...
def check_and_run_daily_reminders_task():
    """
    通过高频轮询（每10分钟，开发环境可配置为每1分钟）动态检查数据库配置，按需触发各类预警和考勤消息。
    
    =============================================================================
    💡 【开发人员指南】系统如何调用并运行“消息通知配置”：
    1. 配置来源：系统运营后台的“系统通知配置”页面。
    2. 数据存储：保存在 system_settings 表中，主键 id='notification_config'，以 JSON 列形式维护每个提醒的属性。
    3. 触发规则：
       - `enabled`: 是否开启该提醒。
    ...
    4. 幂等与时区防护：
       ...
       - 发送成功后立刻将 `last_run_date` 改为今天（'YYYY-MM-DD'），确保当天不再重复推送。
    =============================================================================
    """
    app = create_flask_app_for_task()
    with app.app_context():
        ...
        # 1. 动态加载数据库中的消息通知全局配置，并使用 deepcopy 隔离以确保能够被 SQLAlchemy 的变更追踪检测到
        config_obj = get_or_create_notification_config()
        import copy
        val = copy.deepcopy(config_obj.value)
        ...
        if trial_cfg.get("enabled", True) and current_time_str >= trial_cfg.get("time", "09:00") and trial_cfg.get("last_run_date", "2000-01-01") < today_str:
            ...
            for contract in trials:
                try:
                    ...
                    send_wechat_notification_task.delay(touser=trial_notify_users, title=title, description=desc, jump_url=jump_url, msg_type="TRIAL_EXPIRING")
        ...
        if contract_cfg.get("enabled", True) and current_time_str >= contract_cfg.get("time", "09:00") and contract_cfg.get("last_run_date", "2000-01-01") < today_str:
            ...
            for contract in expirings:
                try:
                    ...
                    send_wechat_notification_task.delay(touser=contract_notify_users, title=title, description=desc, jump_url=jump_url, msg_type="CONTRACT_EXPIRING")
                except Exception as pe:
        ...
        if preg_cfg.get("enabled", True) and current_time_str >= preg_cfg.get("time", "09:00") and preg_cfg.get("last_run_date", "2000-01-01") < today_str:
            ...
            for contract in deliveries:
                try:
                    ...
                    send_wechat_notification_task.delay(touser=preg_notify_users, title=title, description=desc, jump_url=jump_url, msg_type="PREGNANCY_ALERT")
                except Exception as pe:
        ...
        att_notify_users = (att_cfg.get("notify_users") or "").strip() or None
        ...
        # 5. 育儿嫂月度管理费提醒
        mgmt_cfg = reminders.get("monthly_management_fee", {})
        mgmt_notify_users = (mgmt_cfg.get("notify_users") or "").strip() or None
        if mgmt_cfg.get("enabled", True) and current_time_str >= mgmt_cfg.get("time", "09:00") and mgmt_cfg.get("last_run_date", "2000-01-01") < today_str:
            start_day = int(mgmt_cfg.get("start_day", 1) or 1)
            end_day = int(mgmt_cfg.get("end_day", 5) or 5)
            if start_day <= now.day <= end_day:
                from backend.api.billing_api import get_pending_monthly_management_fee_items, _get_default_management_fee_target
                target_year, target_month = _get_default_management_fee_target()
                pending_items = get_pending_monthly_management_fee_items(target_year, target_month)
                if pending_items:
                    frontend_base_url = app.config.get('FRONTEND_BASE_URL', 'http://localhost:5175')
                    jump_url = f"{frontend_base_url}/billing/monthly-management-fees?year={target_year}&month={target_month}"
                    total_due = sum(Decimal(item["total_due"]) for item in pending_items)
                    title = "💰 育儿嫂月度管理费提醒"
                    desc = (
                        f'<div class="gray">账单月份：{target_year}-{target_month:02d}</div>\n'
                        f'<div class="normal">共有 {len(pending_items)} 份育儿嫂合同账单待处理。</div>\n'
                        f'<div class="normal">应付款总额合计：{total_due:.2f} 元。</div>\n'
                        f'<div class="highlight">请向客户发送上月员工工资明细，并催缴本月月度管理费。</div>'
                    )
                    send_wechat_notification_task.delay(
                        touser=mgmt_notify_users,
                        title=title,
                        description=desc,
                        jump_url=jump_url,
        ...
        if changes_made:
            ...
            flag_modified(config_obj, 'value')
            db.session.commit()
...
def send_monthly_attendance_reminder(touser=None):
    """
    月初考勤收集提醒。
    
    ⚠️ 【开发人员注意】此任务由 `check_and_run_daily_reminders_task` 动态判定通过后异步调用。
    请不要在 celeryconfig.py 中配置该任务的静态 crontab 调度，以免产生时区差带来的重复推送幽灵 BUG。
    """
    app = create_flask_app_for_task()
    with app.app_context():
