...
60. 合同管理主要需求：
    1. 创建”制式合同“模板例如月嫂、育儿嫂、试工等合同，每种合同都是三方合同：
        甲方：客户  支付管理费给公司，劳务费给员工
        乙方：员工  收取劳务费
        丙方：我方公司  收取管理费
    现有合同表不能动，也不需要直接在合同表加字段，要增加一个”纵表“的形式来扩充合同字段例如”服务方式“、”服务内容“等
    创建正式的客户数据，并且需要从jinshuju 历史合同中同步已有的客户信息，即甲方信息（姓名，联系电话，身份证号、地址、手写电子签名信息）使用身份证号作为唯一值
    创建员工数据表，同样从jinshuju历史合同中同步已有的员工信息（姓名，联系电话，身份证号，地址，手写电子签名信息），即乙方信息，也使用身份证号作为唯一值，由于同一个员工在不同合同可能存在“劳务费报酬”不一样，员工表只显示最新的劳务费报酬（即员工级别），员工存在“涨薪”的问题，因此要创建一个关联表来记录员工涨薪过程，关联涨薪的合同
    员工需要区分类型，月嫂、育儿嫂等
    以下内容为合同固定内容。
    服务内容
        1. 婴幼儿养护
    ...
    制式合同内容，对不育儿嫂、月嫂此处内容不同。要支持markdown格式的编辑与保存。

    2. 创建合同，选择合同类型（月嫂、育儿嫂、试工）
       2.1 由运营人员填写“乙方信息”，以及乙方劳务报酬（元/月）、保证金（元/月）、丙方管理费（元/月），勾选“是否自动续约”，填写“附件内容”、合同开始时间、合同结束日期。 [Done]
       2.2 在填写“乙方信息”时，要让运营人员操作便捷，通过拼音方式，可以快速检索到系统中已有的员工信息，选择后自动带出乙方姓名、身份证号、身份证上地址、电话号码、劳务报酬、丙方管理费、保证金[Done]
       2.3 创建合同，状态是“未签署”，并生成一个签署链接，并生成一个消息通知模板[Done]
            {客户名称}——{员工名称}：签约2025.11.1~2026.1.31，
            管理费：3个月*950元=2850元，
            客户已交11月管理费：900元，
            增加保证金：500元，
            费用总计：2850-900+500=2450元，
            户名：北京家福安家政服务有限公司
            帐号：8613 8236 7910 001
            银行：招商银行股份有限公司北京万寿路支行

            合同：
            https://contract.mengyimengsao.com/pf/ujSrb1
            1、 请点开上面链接，将甲方内容填写完整
            2、 阅读完内容后，最下面签字、提交
            3、 看见提交成功即可
            ...
            如果合同类型是月嫂，则使用 company_bank_accounts 表中 account_nickname 为 萌姨萌嫂账号 的银行账号，其他合同类型使用 家福安账号对应的银行账号
            在 billing_api.py 中已有此函数和路由
            @billing_bp.route("/company_bank_accounts", methods=["GET"])
            @admin_required
            def get_company_bank_accounts():

      2.4 客户点开上方链接,在手机端查看合同详情，并可以在线进行电子手写签名后点击，确认签署 [Done]
      2.5 此时合同的”签署状态“为“客户已签署”，此时运营人员将此合同发送给员工，有员工进行“乙方人员签署” [Done]
      2.6 员工在线完成合同签署的过程与客户签署过程类似，最周点击“确认签署”，合同的”签署状态“改为“已签署”，合同正式生效，开始按现有逻辑生成账单。 [Done]
      2.7 合同中有公司签章（区分萌姨萌嫂和家福安，分别对应月嫂和育儿嫂合同） [Done]
      2.8 合同列表显示客户、员工签名，取消现有的“客户链接”与“员工链接”按钮[Done]
      2.9 合同详情中增加“下载合同”的按钮，下载同时具有客户、员工签名的pdf格式合同（注意包含合同所有内容）[Done]
      2.10 续签、变更合同以及账单的调整，合同之间的关联。
      
      BUG:
      1. 改了 customer 表结构，之前的客户评价功能不能用了。AI评价也不行了不知道为什么。
...
64. 在“用户管理“中区分一下“未激活”的人不显示，增加筛选项，默认只显示“已激活”的人员[已完成]

65. 将考试结果显示在 mengyimengsao.com 正式简历中，并增加开关，来控制“是否显示考核结果”

66. 设计一个功能，当系统更新到新版后，使用每次的Commit作为 What's NEW 的内容记录下来，以及对应的版本号（每次提交到github，我的提交脚本都会自动打上版本号标签），并在系统中的”历史更新“中可以看到。每次版本升级，用户首次进入系统时，不论在哪个页面，都弹出一个窗口显示What's NEW的内容。

67. 续签合同，财务调整项备注显示以及跳转链接没有显示[Done]

68. 跳转到续签合同详情时，要账单列表是空的，是否因为账单尚未生成？此处为了更好的体验，在创建续签账单的时候可以有一个 “正在创建账单..."的等待，账单创建完毕后再跳转到续签合同详情，这样就可以看到新的账单了！同时合同备注要保持和原合同一致。[Done]

69. 如果是”续签“的合同，在合同详情中显示一个”续签“，并且点击可以跳转到源合同，同样”源合同“详情中也要显示”已续签“，点击后可以跳转到已续签的合同详情中。[Done]

70. 增加“变更”功能，变更时可以换人、修改月薪、管理费率等，变更也要选择变更的后续合同的开始和结束时间。源合同被终止，终止日就是变更的后续合同的开始日。使用已有的“终止合同”功能[Done]

71. 对于续签、变更的合同，要考虑最后一个月账单的费用是否合并到下一个月：[Done]
    续签：
    1. 如果合同结束日不是月末最后一天，则需要调用现有的“合并客户&员工费用到续约账单”的操作，将当前账单的费用全部转移到续约合同的首月账单
    2. 如果合同结束日是月末最后一天，则保持现有逻辑，只转移保证金至续约合同
    变更：
    3. 如果“服务人员”没有变，则与“续约”中的处理一致，按续约中的1、2处理
    4. 如果“服务人员”变更了，则保持现有逻辑，只转移保证金至续约合同。
    5. 变更时要有一个明显的选项“同时转移保证金”，“不转移保证金”，如果选择了“不专一保证金”则不转移任何费用。

72. 合同列表增加”月签“显示以及筛选。[Done]

73. 在创建正式合同的时候，选择一个客户时，要检查一下当前这个客户，在目前是否有可以转移保证金的合同，如果有的话，列出来询问用户是否续签或变更此合同。[Done]
    有了两个选项：
    1. 创建新合同，重新交保证金。
    2. 变更或续约旧合同，可转移保证金。如果有多个合同，列出全部的可转移保证金的合同，包含以下信息，{服务人员}月薪{月薪}的合同于{合同结束日/终止日}结束，有{待转移保证金金额}的保证金可转移。
        2.1 判断”可转移保证金“的合同，此用户的”执行中“(active)或”pending“状态的合同，此用户已终止或结束的合同，并且结束或终止时间是12个月以内的。
    如果选了1，则进入后续正常的填写、创建正式合同的页面，与现在相同
    如果选了2，则跳转到此合同中

74. 从金数据中将签名图片也同步过来并显示，同时更新历史合同的签名状态。[Done] 依靠单独的功能 flask sync-signatures

75. 本地部署一个 HeyForm 看看是否可用，主要是汉化

75. 修改或删除手动创建的正式合同[Done]

76. 合同模板要可以按md格式查看及编辑。[Done]

77. 历史合同也要填充对应的金数据合同模板内容。[Done]

78. 月嫂，替班合同账单需要可以编辑实际工作天数，从而精确管理费与工资。取消只有育儿嫂的合同类型限制 [Done]

79. 对账中心月份切换问题，为空就无法切换了[Done]

80. 合同中金额要有千位分隔符来显示金额。

81. 按现有的合同模板填充到历史合同中。[Done]

82. serveyjs实现表单功能主要是为了历史信息的查看。以及新员工登记[Done]

83. 从金数据已经同步了文字数据下来，图片仍然是链接方式存储的，要研究一个方法：[Done]
    1. 首选图床的方式存储这些小图片
    2. 不行就存本地，扩充硬盘即可
    3. 已使用图床来存储这些图片 [Done]

84. 同步用户时需要同步”地址“到sp表中。[Done]

86. 创建员工信息的时候是否也要同步到 mengyimengsao.com 平台？

87. 签约消息中客户的增加“加和”的过程，员工的只保留“合同”以下的内容。[Done]

88. 围绕“员工登记表”作为核心，进行关联员工合同（与公司与客户），员工信息中编辑、查看合同分别进入员工登记表，以及员工与公司合同的界面
    85. 员工信息中要增加“下户总结”、“入职登记”的表单的内容，关联到员工信息中将上述几个表单，可能还有其他。

89. 填写表单的页面要改成 shadcn ui 的表单样式。 现在界面不好看而且操作到时候是原生的日期选择控件，应该改为手机端的日期选择控件。使用antd -mobile 库以及 drawer 的方式来实现。[Done]

90. 考勤功能优化：[Done] 
    1. 员工填写考勤的页面要支持自动保存，取消保存按钮，只有"提交考勤”按钮，减少按钮对员工带来的”选择困扰“[Done]
    2. 考勤管理月面默认显示的是当前月的上一个月。[Done]
    3. 出勤天数不包含“加班天数”[Done]
    4. 要区分首月考勤表和末月考勤表，首月考勤表要体现上户时间，末月考勤表要体现下户时间。如何定义首月考勤表与末月考勤表？首月账单周期对应的就是首月考勤表，末月账单周期对应的就是末月考勤表。[Done]
        对于首月考勤表，在账单开始日对应的日期前是不可选的，并且不能记录为“正常出勤”，因为此时还没有“上户”。例如账单从5月13日开始，那么考勤表中，5月1日~5月12日期间的日历均不可选，并且不记录为“正常出勤”，日历中用灰色标示并且有一个叉子的icon。
        在首月考勤表中增加一个考勤状态”上户“，并且可以选择具体时间，但只能选择当天的具体时间点，而不是一个时间段，记录员工实际上户时间
        同理，对于末月考勤表，在账单结束日对应的日期后是不可选的，并且不能记录为“正常出勤”，因为这个时间段已经“下户”。例如账单到5月28日结束，那么考勤表中，5月29日~5月31日期间的日历均不可选，并且不记录为“正常出勤”，日历中用灰色标示并且有一个叉子的icon。
        在末月考勤表中增加一个考勤状态”下户“，并且可以选择具体时间，但只能选择当天的具体时间点，而不是一个时间段，记录员工实际上户时间。[Done]
    5. 点击日历后弹出的窗口减少高度，有些手机在微信浏览器中打开时关闭按钮被遮住了，同时在“确认修改”的左侧并排放一个“关闭”按钮，减少此问题的出现。[Done]
    6. 弹窗的时间选择是否可以改用滚筒方式选择时间，而且步长从10分钟改为30分钟，在选择分钟处只能选择00分、30分。[Done]
    7. 有出京和出境的考勤，产生了额外的管理费，账单中添加财务调整项后没有自动重算客户侧的账单以及营收总额[Done]
    8. 下载签署后的考勤表，需要下载到本地，而不是打开浏览器，要包含用户签名。下载的内容与用户签署的界面一致即可。[Done]
    9. 考勤的签署连接分享时使用微信卡片方式分享，显示员工姓名-mm月考勤，点击后跳转到考勤表页面。[Done]
    10. “出勤”天数的计算与显示问题：
        (1) 如果当前月不是合同开始月也不是合同结束月，是“整月”考勤，那么出勤天数做多就是26天
        对于非“自动续签”的育儿嫂合同：
        (2) 非“自动续签”的育儿嫂/月嫂合同，如果当前月是合同开始月，“出勤天数“ = min(当月自然月天数-合同开始日,26)
            非“自动续签”的育儿嫂/月嫂合同，如果当月是合同结束月，“出勤天数”=min(当月自然月天数-合同结束日，26)
        (3) 如果是“自动续签”的育儿嫂合同，只要不是合同开始月，其他月都按(1)中的描述按“整月”处理，出勤天数 = 26
            如果是“自动续签”的育儿嫂合同，当前月是合同的“终止月”（因为自动续签的合同，在合同终止前没有“合同结束月”，因此只有在合同终止时才有合同结束月），“出勤天数”=min(当月自然月天数-合同结束日，26)
        

91. 合同列表按合同更新时间倒排序，例如有客户刚签署了合同，这个合同就应该排在最前面[Done]

92. 现在菜单太多了，后台权限以及角色太单一，需要区分一下管理员、运营、财务、客服、等角色，然后分别赋予权限与菜单。

93. 月嫂”长签“问题，账单周期与育儿嫂是一样的都是30天——延迟解决

94. 考勤表优化：[Done]
    1. 跨天的时候，只有在当天时间为24小时 时才显示相应的考勤类型，否则就是”出勤“。例如请假1天，从3号9点到4号9点，日历上3号显示“请假”4号显示“出勤”。如果是请假3天，从3号9点到6号9点，则需要在3、4、5显示“请假”在6号显示“出勤”。
    2. 考勤列表没有同客户同员工合并或只显示一个新的即可。
    业务上确认了一下，拿休息或请假来举例：
    3. 如果是在当天中午12点前开始的休假或请假，当天日历上就显示”休假或请假“，如果当天中午12点以后开始休息或请假，当天就是”出勤“，而第二天就是”休假或请假“。用此来解决”首日“是否显示”出勤“的问题。
    4. 如果整个假期不满24小时，如果首日晚于中午12点开始，则在第二天显示”休息或请假“不论第二天是几个小时。
    对于第3点和前面的需求有一些冲突，请充分考虑此业务需求。并给出相应的测试用例，尤其是便捷测试
    
    5. 日历上显示法定节假日，以及假期、补班日、工作日等。如果是周六日需要补班的话，就在日期后写一个小的“班”字(注意不换行) (通用api https://timor.tech/api/holiday/year/2025 )
    6. 同一个家庭，不同客户对同一个员工续签合同（如第一个合同是丈夫签署的，第二个续签合同是妻子签署的），此时应该合并该月的考勤表。目前只支持同一个客户在同一个月续签后合并考勤表多功能。
    7. 当前月是12月，员工合同在12月结束，原始的逻辑是员工只能在12月编辑12月考勤，但实际员工在12月也要编辑上个月11月的考勤，此处需要放宽一下条件，
    

95. 员工登记表编辑模式无法删除图片[Done]

96. 动态表单增加面包屑导航，且可点击。[DOne]

97. 微信公众号增加一个”我的考勤“的功能，员工首次点击要求验证姓名与身份证号，核对成功后自动与当前用户的微信openid关联。这样下次点击”我的考勤“时，在公众号直接弹出相应的考勤填写界面，界面显示与员工关联的逻辑与现在一致。[Done]

98. 记录员工的过错与投诉，要建立一个标签库，同一个标签出现两次，就要有惩罚，目前记录在员工登记表中，需要逐渐想员工详情中迁移。
    1. 终止合同时，填写终止原因，自动将此终止记录，关联到员工登记表，以及员工详情中。
    2. 终止合同时，可填写“是否客户投诉”如果各户投诉要选择标签或填写明确投诉内容，记录到员工登记表以及员工详情中。
    3. 员工详情的工作履历中要通过“印章类”效果显示合同状态“已完成、已中止、进行中”等等，如果是“已中止”要显示“终止合同”记录以及终止日期。可以查看合同详情，如果是月签合同未结束/未终止，要显示“月签”不能显示合同结束日（月签不准）。
    4. 下户总结显示在右侧，薪酬变化下方。工作履历中只显示合同，不显示下户总结（最近）

99. 合同详情页面，客户签约消息与员工签约消息的弹窗中，增加一个“查看合同”的连接，点击后可直接查看合同（在新标签中打开）[Done]
100. 生成语音的页面增加一个选项“TTS-Server”，这是自己部署的tts服务，可以支持单句或批量生成语音。可以查看相应的api文档，本地调试[Done]
101. 员工登记表增加自动保存功能，针对同一个手机或同一个用户扫码/点击链接进入后[Done]
102. 考勤中的上户、下户日期关联显示在员工详情的合同信息中，丰富员工信息。如果有上户时间，那么在下户填写下户考勤时显示出对应的上户时间，以便让员工参考。
103. 试工合同在试工成功情况下允许两个分支：
    1. 关联到已有的合同（现有功能）
    2. 允许用户创建新合同，并自动使用当前试工合同的客户信息、员工信息以及月薪，默认是”育儿嫂合同“
104. 考勤中的”上户“那天要算作”出勤“天数，目前没有算为出勤天数。[Done]错，上户那天不计算出勤，与下户那天合在一起计算出勤天数[Done]
105. 美化催款信息需要优化：
    1. 显示加班天数、出勤天数、劳务费总计，而不需要单独显示加班费、基础劳务费等，便于最后一起四舍五入
    2. 天数不要有5.000天这种，如果小数点后面是0就不显示，直接显示5天即可
    3. 增加显示员工银行账号。需要在员工信息中可以编辑
        编辑功能需要增强，手机号、银行账号等基本信息都可以编辑。
106. 催款信息考虑通过小程序呈现给客户
107. 员工信息中如果缺失“银行卡信息”进入小程序后应该有一个“待办事项”，用来补充银行卡信息，持卡人默认是员工自己但可以修改，填写并保存后以此更新到员工表中。以此来增强“催款信息“的显示
...
8. 在财务调整项中，in点击某个调整项，并设置为”已结算“，金额不会计入”实收总额“中，这样在删除这条退款的支付记录时就有问题了！！[已完成]
9. 月嫂替班管理费率没有从页面上获取，而是写死了的25%？[已完成]
10. 试工合同转移时，介绍费2000不应该自动变为已支付。[已完成]
11. 对账中心，账单详情，无法跳转到已转移费用的替班账单[已完成]
12. 同步合同报错[Done]
13. 续签合同时没有继承原合同的合同模板信息、服务内容、服务方式等数据！[Done]
14. 非月签转自动月签时出现了一个莫名其妙的 保证金退款 
15. 周期: 2025-10-17 ~ 2025-11-30 一次交2.5个月的管理费，合同终止在 10月30 日，但是退管理费出现了混乱！终止合同并关联后续合同时也混乱了！
16. 续签变更旧合同时，没有显示关联的合同模板内容以及id，因为源合同是空的，因此新合同继承过来也是空的。要根据合同类型获取一个[Done]
17. 在账单列表中，对于”员工应缴款“计算错误，可能原因是”员工工资单“财务调整项中将”[冲抵]员工待付工资转移至续约合同” 算作了 萌嫂减款 ，导致作为员工应缴款进行了统计。此处需要调整一下或者是修改调整项类型，或者是修改计算“员工应缴款”的逻辑，建议是前者！[Done]
18. 创建合同时修改了”地址“合同中没生效！[Done]
19. 合同签署完毕后，点击”关闭页面“当前页面没有关闭[Done]
20. 编辑月嫂合同时，保证金比例没有从库中读取，而是前端写死了默认值[Done]
21. 合同提前终止，员工工作不满一个月，最后一个月账单显示的“保证金代付工资”应该按最后一个也实际劳务费进行支付（包含加班费）而不是默认一个月工资。
22. 对于x月11日开始y月11日结束的这种非月签合同，在N月M日提前终止时，退还管理费，应该按 x月11日~x+1月10日算一个整月，然后用n+1月10日与终止日n月m日之间的差距来计算待退还管理费的天数，然后加上剩下的整月数（n+1月10日到y月11日，y-(n+1) = 剩余整月数。 按说上述完整逻辑退还管理费。月签合同提前终止退还管理费的算法不变
23. 如果是终止的月签合同，在终止月无法再向后切换考勤月。
24. 月嫂合同，客户签署后仍然显示“客户未签署”[Done]
...
### 3. 前端设计 (Frontend)
https://hr.mengyimengsao.com/attendance-sign/507e505b-0642-4b5a-9413-9f0294966c37?contractId=470e7c99-227f-422c-9b8e-9e8f73f2030c
#### 3.1. UI/UX 流程
...
> **同一字段名「级别」在两类合同中的业务含义不同，不可混用。**

| 合同类型 | 级别业务含义 | 是否包含管理费 | 管理费如何计算 |
| --- | --- | --- | --- |
| **月嫂** | 完整 26 天服务周期的客户应付**总价档位** | **包含**工资 + 管理费 | 从级别总价中按费率拆出：`管理费 = 级别 × 管理费率`，`工资 = 级别 × (1 − 管理费率)` |
| **育儿嫂** | 月度**纯劳务费 / 月薪** | **不包含**管理费 | 在级别之外单独计算并缴纳：`管理费 = 级别 × 管理费率` |
...
*   **核心定义**:
    *   **级别（业务口径）**: 客户在一个完整 **26 天**服务周期内应支付的**总费用档位**，**包含月嫂工资与管理费两部分**。（与育儿嫂「级别」不同：育儿嫂级别仅为纯劳务费）
    *   **工资部分（纯劳务）**: `级别 × (1 − 管理费率)`，即从级别总价中拆出的付给月嫂的劳务报酬。
    *   **管理费（总价拆分口径）**: `级别 × 管理费率` = `客交保证金 − 工资部分`。
    *   **管理费率**: 管理费占级别总价的比例，`管理费 / 级别`。常见 15% / 20% / 25%。
    *   **客交保证金**: 客户实际缴纳的总费用，业务上对应上述**级别总价**（含工资与管理费）。用于在**最后一个服务周期**结清公司管理费 + 月嫂工资，并在合同结束时多退少补。
    *   **员工日薪**: `工资部分 / 26` = `级别 × (1 − 管理费率) / 26`。
    *   **客户日薪**: `级别 / 26`（按含管理费的总价日薪，常用于加班费等客户侧计算）。

*   **「本次交管理费」收取规则（客户账单）**:
    *   **每期均收取**（含末期）：`本次交管理费 = (客交保证金 − 月薪) / 26 × 基本劳务天数`。
    *   末期账单的客户应收总额会同时体现：**本次交管理费**、**保证金退款**、以及（若有可用保证金时的）**代付工资** 等调整项，一并轧差。
    *   批量矫正脚本：`fix_maternity_last_bill_settlement.py`（支持 `--dry-run`）。

*   **系统字段映射（方案 A：只改语义/展示，不改存储）**:
    *   业务「级别 / 总价」→ 存 `security_deposit_paid`（客交保证金）；API 展示字段 `package_level` / `level_display`。
    *   业务「月薪 / 纯劳务」→ 存 `employee_level`；API 展示字段 `salary_amount`。
    *   **切勿**把月嫂 `employee_level` 当成育儿嫂那种「级别」——二者字段同名、语义不同。
    *   拆分口径：`管理费拆分额 = 客交保证金 − employee_level`，`管理费率 = 管理费拆分额 / 客交保证金`。
    *   录入联动（以月薪反推总价）：`客交保证金 = employee_level / (1 − 管理费率)`，`管理费拆分额 = 客交保证金 × 管理费率`。
    *   等价关系：`业务级别 = 客交保证金 = employee_level（月薪）+ 管理费拆分额`。
...
*   **级别（业务总价）**:
    *   **业务含义**: 含工资与管理费的 26 天总价档位。
    *   **系统来源**: 对应 `MaternityNurseContract.security_deposit_paid`（客交保证金）；工资部分对应 `BaseContract.employee_level`。
*   **定金**:
    *   **来源**: `MaternityNurseContract.deposit_amount`
*   **客交保证金**:
    *   **来源**: `MaternityNurseContract.security_deposit_paid`（即业务级别总价）
*   **劳务费时段**:
    *   **计算**: 根据“账单周期计算逻辑”确定。
*   **基本劳务天数**:
    *   **显示**: `min(账单周期,26天)`
...
*   **被替班天数**:
    *   **计算**: 其他员工代替此阿姨的“替班天数”
*   **基础劳务费**:
    *   **计算**: `员工日薪 * 基本劳务天数 (即 26)`（员工日薪按**工资部分**）
*   **加班费**:
    *   **计算**: `客户日薪 * 加班天数`（客户日薪按**级别总价**）
*   **本次交管理费**:
    *   **含义**: 本账单周期客户应向公司支付的管理费（**含末期**，与首期/中间期同一公式）。
    *   **计算**: `(客交保证金 − employee_level) / 26 × 基本劳务天数`。
*   **管理费率：**
    *   **计算：**`(客交保证金 − employee_level) / 客交保证金`
*   **优惠**:
    *   **来源**: `MaternityNurseContract.discount_amount`
*   **客增加款 / 退客户款**:
    *   **计算**: 汇总相应类型的 `FinancialAdjustment` 金额。
*   **客应付款**:
    *   **计算**: 以管理费等公司侧费用为主：`本次交管理费 + 客户增款(含代付工资等) − 客户减款(含保证金退款) − 优惠 + …`。
    *   **末期**: 仍有「本次交管理费」；同时有「保证金退款」及（有可用保证金时）「公司代付工资」，轧差后得到应收。
*   **末期「保证金代付工资」**（仅此额外调整项，有前提）:
    *   **前提**：本期仍有可用客交保证金（未转出）。
    *   生成客户侧 `[系统] 公司代付工资` + 员工侧 `[系统] 保证金支付工资`。
    *   **不**再单独生成「保证金代付管理费」调整项；管理费以「本次交管理费」字段体现。
    *   保证金已转出或工资已整笔转移后继合同：不生成代付工资。
*   **定金**: `AdjustmentType.DEPOSIT` 计入客户账单应收（通常挂首期）。
...
*   **总劳务天数**:
    *   **计算**: `26 + 加班天数 - 被替班天数  `
*   **萌嫂保证金(工资)**:
    *   **计算**: `员工日薪 * 基本劳务天数 (即 26)`（按**工资部分**，不含管理费）
*   **加班费**:
    *   **计算**: `客户日薪 * 加班天数`
*   **5%奖励**:
    *   **条件**: `management_fee_rate` 为 15%。
    *   **计算**: `工资部分 * 5%`（实现上为 `employee_level * 5%`）
...
*   **核心定义**:
    *   **级别 (`employee_level`，业务口径)**: 育儿嫂的**月度基础劳务费 / 月薪，仅指纯劳务费，不包含管理费**。（与月嫂「级别」不同：月嫂级别是含管理费的总价档位）
    *   育儿嫂的管理费在级别之外**单独计算并缴纳**，不从级别中拆分。
    *   自动续签(is_monthly_auto_renew)：代表合同结束后，自动按自然月续约（每个月1日到当月最后一天）。
    *   管理费率：默认 10%
    *   管理费： `级别 * 管理费费率`（例如 `级别 × 10%`）
    *   **客户日薪**: `级别 / 26`（因级别本身不含管理费，客户侧与员工侧日薪相同）
    *   **员工日薪**: `级别 / 26`
*   **账单周期 (`本月合同时间段`)**:
...
#### 	月嫂：
	...
	> 此处「级别」与月嫂正式合同业务口径一致：**含工资 + 管理费的总价**，再按费率拆分。

	管理费率 = `25%` 管理费率由运营人员选择15%或25%，此处默认值是25%。
	
	替班日薪（工资部分） = `B阿姨的级别 * (1-管理费率) / 26` 
	
	基础服务费 = `B阿姨的级别 * (1-管理费率) / 26 * 替班天数`
	
	管理费 = `B阿姨的级别 * 管理费率 / 26 * 替班天数`
	
	客户应付 = `基础服务费 + 管理费` = `B阿姨的级别 / 26 * 替班天数` = 被替班扣款（此金额已包含管理费）
	
	月嫂工资 = `B阿姨的级别 * (1-管理费率) / 26 * 替班天数` 。

#### 	育儿嫂：

	> 此处「级别」与育儿嫂正式合同业务口径一致：**纯劳务费 / 月薪，不包含管理费**。
    ...
    合同期外替班管理费 = `B阿姨的级别 * 合同期外管理费率 ` / 30 * 替班天数 ` （和育儿嫂合同一样，按30天计算管理费；管理费在级别之外另算）
...
2.  **【高优先级】实现替班功能增强与账单周期顺延**:[已完成]
    ...
    *   **育儿嫂替班人员的计费规则:**
        *   **级别口径**: 纯劳务费 / 月薪，不包含管理费（与育儿嫂正式合同业务口径一致）。
        *   管理费率：`0` (育儿嫂替班不收取管理费)。
        *   替班日薪：`B阿姨的级别 / 26`。
        *   客户应付：`B阿姨的级别 / 26 * 替班天数`。
        *   员工工资：`B阿姨的级别 / 26 * 替班天数`。
