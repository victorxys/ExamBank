...
89. 填写表单的页面要改成 shadcn ui 的表单样式。 现在界面不好看而且操作到时候是原生的日期选择控件，应该改为手机端的日期选择控件。使用antd -mobile 库以及 drawer 的方式来实现。
